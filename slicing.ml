...
let dependencies =
  Command.basic ~summary:"Produce a PDG for a given function"
    Command.Let_syntax.(
      ...
      fun () ->
        ...
        let control_annot =
          Control_deps.annotate_exact
            (Cfg.without_empty_nodes_with_no_predecessors cfg)
        in
        Out_channel.with_file dot_filename ~f:(fun ch ->
            Out_channel.output_string ch
              (Cfg.to_dot cfg ~annot_str:Spec.to_dot_string
                 ~extra_data:(use_def_annot ^ control_annot))))

let cdg =
  Command.basic ~summary:"Produce a CDG for a given function"
    Command.Let_syntax.(
      let%map_open filename = anon ("file" %: string)
      and funidx = anon ("fun" %: int32)
      and dot_filename = anon ("out" %: string) in
      fun () ->
        ...
        Out_channel.with_file dot_filename ~f:(fun ch ->
            Out_channel.output_string ch
              (Cfg.to_dot cfg ~include_edges:false ~extra_data:control_annot)))

let postdom =
  Command.basic ~summary:"Visualize the post-dominator tree of a function"
    Command.Let_syntax.(
      let%map_open filename = anon ("file" %: string)
      and funidx = anon ("fun" %: int32)
      and dot_filename = anon ("out" %: string) in
...
let slice =
  Command.basic
    ~summary:
      "Produce an executable program after slicing the given function at the \
      ...
      fun () ->
        ...
        let cfg =
          Cfg.without_empty_nodes_with_no_predecessors
            (Spec_analysis.analyze_intra1 module_ funidx)
        in
        let slicing_criterion =
          Instr.Label.{ section = Function funidx; id = instr }
        in
        Log.info "Slicing";
        Log.info
          (Printf.sprintf "Slicing criterion: %s"
        ...
        Out_channel.with_file outfile ~f:(fun ch ->
  ...
  Command.basic
    ...
    Command.Let_syntax.(
      ...
      fun () ->
        ...
        let instr =
          match
            List.find (Cfg.all_instructions_list cfg) ~f:(fun instr ->
                Instr.line_number instr = line_number)
          with
