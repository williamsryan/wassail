...
let () =
  Js.export "jsbridge"
    (object%js (self)
       ...
       method load (program : Js.js_string Js.t) : unit =
         self##.program := Some (Js.to_string program)
       ...
       method cfgs : 'a (* an object mapping function indices to CFGs *) =
         ...
         let cfgs_alist =
           List.map (Int32Map.to_alist cfgs) ~f:(fun (id, cfg) ->
               (Int32.to_string id, Js.Unsafe.inject (create_cfg cfg)))
         in
