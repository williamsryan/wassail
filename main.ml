  ...
  Command_unix.run ~version:"0.0"
    (Command.group ~summary:"Static analysis of WebAssembly"
       [
         ...
         ("count", Utils.count) (* Utilities that require building the CFGs *);
         ("cfg", Cfg.cfg);
         ...
         ( "taint-from-exported-to-imported",
           Analysis.taint_flow_from_exported_to_imported );
         ( "taint-from-sources-to-sinks",
           Analysis.taint_flow_from_sources_to_sinks );
         ("taintcall-cfg", Analysis.taintcall_cfg);
         ("find-indirect-calls", Analysis.find_indirect_calls) (* Slicing *);
