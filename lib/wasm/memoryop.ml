...
module T = struct
  type extension = SX | ZX [@@deriving sexp, compare, equal]
...
let pack_size_to_int (p : pack_size) : int =
  match p with Pack8 -> 8 | Pack16 -> 16 | Pack32 -> 32 | Pack64 -> 64
...
let size (op : t) : int32 =
  match op.pack with
  | None -> (
      match Type.size op.typ with 32l -> 4l | 64l -> 8l | _ -> assert false)
  | Some (Pack8, _) -> 1l
  | Some (Pack16, _) -> 2l
  | Some (Pack32, _) -> 4l
  | Some (Pack64, _) -> 8l
...
let to_string (op : t) : string =
  ...
  let align =
    if Int32.(align_exp = size op) then ""
    else Printf.sprintf "align=%ld" align_exp
  in
  let sep =
    if String.is_empty offset || String.is_empty align then "" else " "
  in
