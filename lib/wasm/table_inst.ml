...
module T = struct
  type funcelem = Int32.t option [@@deriving sexp, compare, equal]

  type t = {
    elems : funcelem Array.t;  (** The elements in the table *)
    max : Int32.t option;
        (** Maximal size of the table. If None, it is unlimited *)
  }
...
let init (t : Table.t) (elems : Elem_segment.t list) : t =
  let table =
    {
      max = Limits.max t.ttype;
      elems = Wasm.Lib.Array32.make (Limits.min t.ttype) None;
    }
  in
          ...
          | Data { instr = Const (I32 offset); _ } :: [] ->
              List.iteri e.einit ~f:(fun idx addrinstrs ->
                  ...
                  | Data { instr = Const (I32 addr); _ } :: [] ->
                      Wasm.Lib.Array32.set table.elems
                        Int32.(of_int_exn idx + offset)
                        (Some addr)
...
let to_string (t : t) : string =
  Printf.sprintf "elems: %s\n"
    (String.concat ~sep:","
       (List.map (Array.to_list t.elems) ~f:(function
         | None -> "/"
