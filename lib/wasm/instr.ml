...
module Label = struct
  ...
  type section =
    | Function of Int32.t
        (** Instruction is part of the function with the given index *)
    | Elem of Int32.t
        (** Instruction is part of table elements with the given index *)
    | Data of Int32.t
        (** Instruction is part of data element with the given index *)
    | MergeInFunction of Int32.t
        (** Instruction is a merge instruction in the given function *)
    | Dummy
        (** Dummy section, introduced by an extra processing phase (e.g., slicing) *)
  [@@deriving sexp, compare, equal]

  let section_to_string (s : section) =
    match s with
  ...
  module T = struct
    type t = { section : section; id : int } [@@deriving sexp, compare, equal]
    (** A label is a unique identifier for an instruction *)

    let to_string (l : t) : string =
      match l.section with
      | Function _ ->
          Printf.sprintf "%d"
            l.id (* Printed differently to have a cleaner output *)
  ...
  module Set = struct
    module T = struct
      ...
      let to_string (t : t) : string =
        String.concat ~sep:"," (List.map ~f:T.to_string (Set.to_list t))
    end
  ...
  (** Test data *)
  module Test = struct
    let lab ?(fidx : int32 = 0l) (n : int) = { section = Function fidx; id = n }

    let merge ?(fidx : int32 = 0l) (n : int) =
      { section = MergeInFunction fidx; id = n }
  end
end
...
module T = struct
  ...
  type arity = int * int [@@deriving sexp, compare, equal]
  ...
  type block_type = Type.t option [@@deriving sexp, compare, equal]
  ...
  and 'a control =
    ...
    | Unreachable
    | Merge
  (* Special instruction not existing in Wasm, used to handle control-flow merges *)
  ...
  (** All instructions *)
  and 'a t = Data of 'a labelled_data | Control of 'a labelled_control
  [@@deriving sexp, compare, equal]
end

include T

let is_block (instr : 'a t) : bool =
  match instr with
...
let label (instr : 'a t) : Label.t =
  match instr with Data i -> i.label | Control i -> i.label
...
let data_to_string (instr : data) : string =
  ...
  | Const v ->
      Printf.sprintf "%s.const %s"
        (Type.to_string (Prim_value.typ v))
        (Prim_value.to_string v)
  ...
  | Load op ->
      ...
      Printf.sprintf "%s.load%s%s%s" (Type.to_string op.typ)
        (Memoryop.suffix_to_string op true)
        (if String.is_empty memop then "" else " ")
        memop
  | Store op ->
      let memop = Memoryop.to_string op in
      Printf.sprintf "%s.store%s%s%s" (Type.to_string op.typ)
...
let rec control_to_string ?(sep : string = "\n") ?indent:(i : int = 0)
    ?annot_str:(annot_to_string : 'a -> string = fun _ -> "")
    (instr : 'a control) : string =
  match instr with
  ...
  | BrTable (t, b) ->
      Printf.sprintf "br_table %s %s"
        (String.concat ~sep:" " (List.map t ~f:Int32.to_string))
        (Int32.to_string b)
  | Return -> "return"
  | Unreachable -> "unreachable"
  | Block (bt, _, instrs) ->
      Printf.sprintf "block%s%s%s%s%send" (block_type_to_string bt) sep
        (list_to_string instrs annot_to_string ~indent:(i + 2) ~sep)
        sep (String.make i ' ')
  | Loop (bt, _, instrs) ->
      Printf.sprintf "loop%s%s%s%s%send" (block_type_to_string bt) sep
        (list_to_string instrs annot_to_string ~indent:(i + 2) ~sep)
        sep (String.make i ' ')
  | If (bt, _, instrs1, []) ->
      Printf.sprintf "if%s%s%s%s%send" (block_type_to_string bt) sep
        (list_to_string instrs1 annot_to_string ~indent:(i + 2) ~sep)
        sep (String.make i ' ')
  | If (bt, _, instrs1, instrs2) ->
      Printf.sprintf "if%s%s%s%s%selse%s%s%s%send" (block_type_to_string bt) sep
        (list_to_string instrs1 annot_to_string ~indent:(i + 2) ~sep)
        sep (String.make i ' ') sep
        (list_to_string instrs2 annot_to_string ~indent:(i + 2) ~sep)
        sep (String.make i ' ')
  | Merge -> "merge"

(** Converts an instruction to its string representation *)
and to_string ?(sep : string = "\n") ?indent:(i : int = 0)
    ?annot_str:(annot_to_string : 'a -> string = fun _ -> "") (instr : 'a t) :
    string =
  Printf.sprintf "%s%s" (String.make i ' ')
    (match instr with
    | Data instr -> data_to_string instr.instr
    | Control instr ->
        control_to_string instr.instr ~annot_str:annot_to_string ~sep ~indent:i)
(* (Label.to_string (label instr)) *)

and list_to_string ?indent:(i : int = 0) ?(sep : string = ", ") (l : 'a t list)
    (annot_to_string : 'a -> string) : string =
  String.concat ~sep
    (List.map l ~f:(fun instr ->
         to_string instr ~annot_str:annot_to_string ?sep:(Some sep)
           ?indent:(Some i)))

module Set = struct
  module TUnit = struct
    type t = unit T.t [@@deriving sexp, compare, equal]
  ...
  module T = struct
    ...
    let to_string (t : t) : string =
      String.concat ~sep:"," (List.map ~f:TUnit.to_string (Set.to_list t))
  end
...
let to_mnemonic (instr : 'a t) : string =
  match instr with
  | Data d -> (
      match d.instr with
      | Nop -> "nop"
...
let rec of_wasm (m : Wasm.Ast.module_) (new_label : unit -> Label.t)
    (i : Wasm.Ast.instr) : unit t =
  ...
  let data_labelled ?label:(lab : Label.t option) (instr : data) : unit t =
    let label = match lab with Some l -> l | None -> new_label () in
    Data
      {
        instr;
        label;
  ...
  | Block (st, instrs) ->
      ...
      assert (arity_in = 0);
      (* what does it mean to have arity_in > 0? *)
      assert (arity_out <= 1);
      let label = new_label () in
      let body = seq_of_wasm m new_label instrs in
      control_labelled ~label (Block (block_type, (arity_in, arity_out), body))
  | Const lit -> data_labelled (Const (Prim_value.of_wasm_num lit.it))
  | Binary bin -> data_labelled (Binary (Binop.of_wasm bin))
  | Compare rel -> data_labelled (Compare (Relop.of_wasm rel))
  | LocalGet l -> data_labelled (LocalGet l.it)
  | LocalSet l -> data_labelled (LocalSet l.it)
  | LocalTee l -> data_labelled (LocalTee l.it)
  | BrIf label -> control_labelled (BrIf label.it)
  | Br label -> control_labelled (Br label.it)
  | BrTable (table, label) ->
  ...
  | Call f ->
      ...
      if arity_out > 1 then
        failwith
          "Unsupported: direct function call with more than one return value";
      control_labelled (Call ((arity_in, arity_out), t, f.it))
  | Return -> control_labelled Return
  | Unreachable -> control_labelled Unreachable
  | Select types ->
      data_labelled (Select (Option.map ~f:(List.map ~f:Type.of_wasm) types))
  | Loop (st, instrs) ->
      let arity_in, arity_out = Wasm_helpers.arity_of_block m st in
      assert (arity_in = 0);
      (* what does it mean to have arity_in > 0 for a loop? *)
      assert (arity_out <= 1);
      (* TODO: support any arity out? *)
      let label = new_label () in
      let body = seq_of_wasm m new_label instrs in
      control_labelled ~label
        (Loop (Wasm_helpers.type_of_block m st, (arity_in, arity_out), body))
  | If (st, instrs1, instrs2) ->
      ...
      control_labelled ~label
        (If
           (Wasm_helpers.type_of_block m st, (arity_in, arity_out), body1, body2))
  | CallIndirect (table, f) ->
      let (arity_in, arity_out), t =
        Wasm_helpers.arity_and_type_of_fun_type m f
      in
      if arity_out > 1 then
        failwith
          "Unsupported: indirect function call with more than one return value"
      else
        control_labelled
          (CallIndirect (table.it, (arity_in, arity_out), t, f.it))
  | GlobalGet g -> data_labelled (GlobalGet g.it)
  | GlobalSet g -> data_labelled (GlobalSet g.it)
  | Load op -> data_labelled (Load (Memoryop.of_wasm_load op))
  | Store op -> data_labelled (Store (Memoryop.of_wasm_store op))
  ...
  | Test op -> data_labelled (Test (Testop.of_wasm op))
  | Convert op -> data_labelled (Convert (Convertop.of_wasm op))
  | Unary op -> data_labelled (Unary (Unop.of_wasm op))
  | RefNull t -> data_labelled (RefNull (Ref_type.of_wasm t))
  | RefFunc f -> data_labelled (RefFunc f.it)
  | RefIsNull -> data_labelled RefIsNull
  | VecLoad _ | VecStore _ | VecLoadLane _ | VecStoreLane _ | VecConst _
  | VecTest _ | VecCompare _ | VecUnary _ | VecBinary _ | VecConvert _
  | VecShift _ | VecBitmask _ | VecTestBits _ | VecUnaryBits _ | VecBinaryBits _
  | VecTernaryBits _ | VecSplat _ | VecExtract _ | VecReplace _ ->
      Unsupported.vector_type ()
  | TableGet _ | TableSet _ | TableGrow _ | TableFill _ | TableCopy _
  | TableInit _ | TableSize _ ->
      Unsupported.table_instructions ()
  | ElemDrop _ | DataDrop _ -> Unsupported.drop_2_instructions ()

(** Creates a sequence of instructions from their Wasm representation *)
and seq_of_wasm (m : Wasm.Ast.module_) (new_label : unit -> Label.t)
    (is : Wasm.Ast.instr list) : unit t list =
...
and map_annotation_data (i : (data, 'a) labelled) ~(f : 'a t -> 'b * 'b) :
    (data, 'b) labelled =
  let annotation_before, annotation_after = f (Data i) in
  { i with annotation_before; annotation_after }

and map_annotation_control (i : ('a control, 'a) labelled)
    ~(f : 'a t -> 'b * 'b) : ('b control, 'b) labelled =
  let annotation_before, annotation_after = f (Control i) in
  {
    ...
    instr =
      (match i.instr with
      | Block (bt, arity, instrs) ->
          Block (bt, arity, List.map instrs ~f:(map_annotation ~f))
      | Loop (bt, arity, instrs) ->
          Loop (bt, arity, List.map instrs ~f:(map_annotation ~f))
      | If (bt, arity, then_, else_) ->
          If
            ( bt,
              arity,
              List.map then_ ~f:(map_annotation ~f),
              List.map else_ ~f:(map_annotation ~f) )
...
and drop_labels_control (i : ('a control, 'a) labelled) :
    ('a control, 'a) labelled =
  {
    i with
    label = Label.{ section = Dummy; id = 0 };
    line_number = 0;
    instr =
      (match i.instr with
      | Block (bt, arity, instrs) ->
          Block (bt, arity, List.map instrs ~f:drop_labels)
      | Loop (bt, arity, instrs) ->
          Loop (bt, arity, List.map instrs ~f:drop_labels)
      | If (bt, arity, then_, else_) ->
          If
            ( bt,
              arity,
...
let clear_annotation_control (i : ('a control, 'a) labelled) :
    (unit control, unit) labelled =
...
let annotation_after (i : 'a t) : 'a =
  match i with Data d -> d.annotation_after | Control c -> c.annotation_after
...
let rec all_labels_no_blocks (i : 'a t) : Label.Set.t =
  ...
  | Control c -> (
      match c.instr with
      | Block (_, _, instrs) | Loop (_, _, instrs) ->
          List.fold_left instrs ~init:Label.Set.empty ~f:(fun acc i ->
              Label.Set.union acc (all_labels_no_blocks i))
      | If (_, _, instrs1, instrs2) ->
          List.fold_left (instrs1 @ instrs2) ~init:Label.Set.empty
            ~f:(fun acc i -> Label.Set.union acc (all_labels_no_blocks i))
...
let rec all_labels_no_merge (i : 'a t) : Label.Set.t =
  ...
  | Control c -> (
      match c.instr with
      | Block (_, _, instrs) | Loop (_, _, instrs) ->
          List.fold_left instrs ~init:(Label.Set.singleton c.label)
            ~f:(fun acc i -> Label.Set.union acc (all_labels_no_merge i))
      | If (_, _, instrs1, instrs2) ->
          List.fold_left (instrs1 @ instrs2) ~init:(Label.Set.singleton c.label)
            ~f:(fun acc i -> Label.Set.union acc (all_labels_no_merge i))
...
let rec all_labels (i : 'a t) : Label.Set.t =
  ...
  | Control c ->
      Label.Set.add
        ...
        | If (_, _, instrs1, instrs2) ->
            List.fold_left (instrs1 @ instrs2) ~init:Label.Set.empty
              ~f:(fun acc i -> Label.Set.union acc (all_labels i))
        | _ -> Label.Set.empty)
        c.label

let rec contains ~(f : 'a t -> bool) (instr : 'a t) : bool =
...
let instructions_contained_in (i : 'a t) : 'a t list =
  match i with
  | Data _ -> []
  | Control c -> (
      match c.instr with
      | Block (_, _, instrs) | Loop (_, _, instrs) -> instrs
      | If (_, _, instrs1, instrs2) -> instrs1 @ instrs2
      | _ -> [])

let line_number (i : 'a t) : int =
  match i with Data d -> d.line_number | Control c -> c.line_number
