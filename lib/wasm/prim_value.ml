...
module T = struct
  type t = I32 of int32 | I64 of int64 | F32 of Wasm.F32.t | F64 of Wasm.F64.t
...
let is_zero (v : t) : bool = match v with I32 0l | I64 0L -> true | _ -> false
let is_one (v : t) : bool = match v with I32 1l | I64 1L -> true | _ -> false

let is_positive (v : t) : bool =
  match v with
  | I32 n -> Int32.(n > 0l)
  | I64 n -> Int64.(n > 0L)
  | F32 _ | F64 _ -> failwith "unsupported"

let is_negative (v : t) : bool =
  match v with
  | I32 n -> Int32.(n < 0l)
  | I64 n -> Int64.(n < 0L)
  | F32 _ | F64 _ -> failwith "unsupported"

let byte_of (v : t) (byte : int) =
  match (v, byte) with
...
let zero_of_same_t (v : t) : t =
  match v with
  | I32 _ -> I32 0l
  | I64 _ -> I64 0L
  | F32 _ | F64 _ -> failwith "unsupported"

let of_int_t (t : Type.t) (n : int) : t =
  match t with
...
let lift_wasm_bin (op32 : Wasm.I32.t -> Wasm.I32.t -> Wasm.I32.t)
    (op64 : Wasm.I64.t -> Wasm.I64.t -> Wasm.I64.t) : t -> t -> t =
 fun v1 v2 ->
  match (v1, v2) with
...
let lift_wasm_un (op32 : Wasm.I32.t -> Wasm.I32.t)
    (op64 : Wasm.I64.t -> Wasm.I64.t) : t -> t = function
  | I32 x -> I32 (op32 x)
  | I64 x -> I64 (op64 x)
  | F32 _ | F64 _ -> failwith "unsupported"

let lift_wasm_test (op32 : Wasm.I32.t -> bool) (op64 : Wasm.I64.t -> bool) :
    t -> bool = function
  | I32 x -> op32 x
  | I64 x -> op64 x
  | F32 _ | F64 _ -> failwith "unsupported"

let lift_wasm_rel (op32 : Wasm.I32.t -> Wasm.I32.t -> bool)
    (op64 : Wasm.I64.t -> Wasm.I64.t -> bool) : t -> t -> bool =
 fun v1 v2 ->
  match (v1, v2) with
