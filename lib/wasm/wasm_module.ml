...
module T = struct
  type t = {
    ...
    types : (Type.t list * Type.t list) list;
        (** The types declared in the module *)
    imported_globals : Global.t list;  (** The imported globals *)
    imported_global_types : Type.t list;
        (** The types of the imported globals *)
    ...
    imported_funcs : (Int32.t * string * (Type.t list * Type.t list)) list;
        (** The description of the imported function: their id, name, and type *)
    exports : Export.t list;
    exported_funcs : (Int32.t * string * (Type.t list * Type.t list)) list;
        (** The description of the exported functions: the id, name, and type *)
    ...
    datas : Data_segment.t list;
        (** The data segments containing initial data *)
...
let get_funcnames (m : t) : int32 StringMap.t =
  let alist =
    List.filter_map m.funcs ~f:(fun funcinst ->
        Option.map funcinst.name ~f:(fun name -> (name, funcinst.idx)))
  in
  let imported =
    List.mapi
      (List.filter m.imports ~f:(function
        ...
        | _ -> false))
      ~f:(fun idx f -> (f.item_name, Int32.of_int_exn idx))
  in
...
let replace_func (m : t) (fidx : Int32.t) (finst : Func_inst.t) : t =
  {
    m with
    funcs =
      List.map m.funcs ~f:(fun f -> if Int32.(f.idx = fidx) then finst else f);
  }

(** Constructs a Wasm_module *)
let of_wasm (m : Wasm.Ast.module_) : t =
  let imported_funcs =
    List.filter_mapi m.it.imports ~f:(fun idx import ->
        match import.it.idesc.it with
        | FuncImport v ->
            Some
              ( Int32.of_int_exn idx,
                Wasm.Ast.string_of_name import.it.item_name,
  ...
  let imported_globals =
    List.filter_map m.it.imports ~f:(fun import ->
        match import.it.idesc.it with
  ...
  let imported_global_types =
    List.map imported_globals ~f:(fun g -> g.gtype.typ)
  in
  let globals =
    List32.mapi m.it.globals ~f:(fun idx g ->
        Global.of_wasm m Int32.(idx + nimported_globals) g)
  in
  let nglobals = List32.length m.it.globals in
  let global_types =
    List.map m.it.globals ~f:(fun g ->
        match g.it.gtype with Wasm.Types.GlobalType (t, _) -> Type.of_wasm t)
  in
  let funcs =
    List32.mapi m.it.funcs ~f:(fun i f ->
        Func_inst.of_wasm m Int32.(i + nfuncimports) f)
  in
  let ftype (fidx : Int32.t) : Type.t list * Type.t list =
    if Int32.(fidx < nfuncimports) then
      ...
      | Some (_, _, typ) -> typ
      | None ->
          failwith "of_wasm: nth error when looking for imported function type"
    else
      ...
      | None ->
          failwith
            (Printf.sprintf
               "of_wasm: nth error when looking for function type (function \
                %ld unfound in type list of length %ld)"
               Int32.(fidx - nfuncimports)
               (List32.length funcs))
  in
  let exports = List.map m.it.exports ~f:Export.of_wasm in
  let exported_funcs =
    List.filter_map m.it.exports ~f:(fun export ->
  ...
  let memory_insts =
    List.filter_map m.it.imports ~f:(fun import ->
        ...
        | _ -> None)
    @ List.map m.it.memories ~f:Memory_inst.of_wasm
  in
  ...
  let table_insts =
    List.map m.it.tables ~f:(fun t ->
        Table_inst.init (Table.of_wasm t)
          (List32.mapi m.it.elems ~f:(Elem_segment.of_wasm m)))
  in
  ...
  let elems = List32.mapi m.it.elems ~f:(Elem_segment.of_wasm m) in
  {
    ...
    table_insts;
    types =
      List.map m.it.types ~f:(fun t ->
          match t.it with
          | Wasm.Types.FuncType (a, b) ->
              (List.map a ~f:Type.of_wasm, List.map b ~f:Type.of_wasm));
  }

let of_file (file : string) : t = apply_to_file file of_wasm
let of_string (string : string) : t = apply_to_string string of_wasm
...
let to_string (m : t) : string =
  ...
  let limits (l : Limits.t) =
    match l with
    | low, None -> put (Printf.sprintf "%ld" low)
    | low, Some high -> put (Printf.sprintf "%ld %ld" low high)
  in
  ...
  let import (i : int) (import : Import.t) =
    put
      (Printf.sprintf "  (import \"%s\" \"%s\" " import.module_name
         import.item_name);
    (match import.idesc with
    | FuncImport tidx -> put (Printf.sprintf "(func (;%d;) (type %ld))" i tidx)
    | TableImport _ ->
        ()
        (* failwith "Unsupported: table import" (* should be easy to add, just need an example *) *)
    | MemoryImport l ->
    ...
    | GlobalImport { typ; _ } ->
        put
          (Printf.sprintf "(global (;%d;) %s)" (next_global_import ())
             (Type.to_string typ)));
    put ")\n"
  in
  ...
  let func (i : Int32.t) (f : Func_inst.t) =
    ...
    put
      (Instr.list_to_string f.code.body ?indent:(Some 4) ?sep:(Some "\n")
         (fun () -> ""));
    put ")\n"
  in
  let funcs () =
    List.iteri m.funcs ~f:(fun i f ->
        func Int32.(m.nfuncimports + of_int_exn i) f)
  in
  ...
  let elem (elem : Elem_segment.t) =
    ...
    put
      (Printf.sprintf "(%s)"
         (Instr.list_to_string (Segment_mode.offset elem.emode) (fun () -> "")));
    (* TODO: this is not the general case, check that this is enough *)
    put
      (Printf.sprintf " func %s)\n"
         (String.concat ~sep:" "
            (List.map
               ~f:(fun l -> Instr.list_to_string l (fun () -> ""))
               elem.einit)))
  in
  ...
  let data (data : Data_segment.t) =
    ...
    put
      (Printf.sprintf "(offset %s)"
         (Instr.list_to_string (Segment_mode.offset data.dmode) (fun () -> "")));
    put "\"";
    put (string_to_wasm_string data.dinit);
    (* TODO: make sure to escape what is needed *)
    put "\")\n"
  in
  let datas () = List.iter m.datas ~f:data in
  let global (i : int) (g : Global.t) =
    put
      (Printf.sprintf "  (global (;%d;) %s" i
  ...
  let%test_unit "construct Wasm_module from .wat files without erroring" =
    List.iter
      [
        ...
        with e ->
          failwith
            (Printf.sprintf "Cannot parse %s: %s" program (Exn.to_string e)))

  let%test "get_func_type with indirect call to imported function" =
    let m : t =
      of_file "../../../test/call_indirect-with_imported_element.wat"
    in
    let t1, t2 = get_func_type m 0l in
    List.is_empty t1
    && List.length t2 = 1
    && Type.equal (List.hd_exn t2) Type.I32
end
