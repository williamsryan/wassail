...
(* Map from instruction to its memory dependencies *)
...
let make (cfg : 'a Cfg.t) : t =
  ...
  let loads_and_calls =
    Instr.Label.Map.keys
      (Instr.Label.Map.filter instrs ~f:(fun i ->
           match i with
  ...
  let deps =
    Instr.Label.Map.of_alist_exn
      (List.map loads_and_calls ~f:(fun label ->
           let block = Cfg.find_enclosing_block_exn cfg label in
           let predecessors = Cfg.all_predecessors cfg block in
           ( label,
             List.fold_left predecessors ~init:Instr.Label.Set.empty
               ~f:(fun acc block ->
                 Instr.Label.Set.union acc
                   (match block.content with
                   | Control { instr = Call _; label; _ } ->
                       Instr.Label.Set.singleton label
                   | Control { instr = CallIndirect _; label; _ } ->
                       Instr.Label.Set.singleton label
