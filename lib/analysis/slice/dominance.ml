...
module Graph = struct
  ...
  let of_alist_exn (entry : int) (exit : int) (desc : (int * int list) list) :
      'a t =
    let nodes = List.concat_map desc ~f:(fun (from, to_) -> from :: to_) in
    let map : int list IntMap.t = IntMap.of_alist_exn desc in
    let succs n = match IntMap.find map n with Some ns -> ns | None -> [] in
    let rev_map : int list IntMap.t =
      List.fold_left desc ~init:IntMap.empty ~f:(fun acc (k, vs) ->
          List.fold_left vs ~init:acc ~f:(fun acc v ->
              IntMap.add_multi acc ~key:v ~data:k))
    in
    let preds n =
      match IntMap.find rev_map n with Some ns -> ns | None -> []
    in
    { entry; nodes; succs; preds; exit }

  (** Computes a spanning tree *)
  let spanning_tree (graph : int t) : Tree.t =
    let rec dfs (tree : int list IntMap.t) (added : IntSet.t) (node : int) :
        int list IntMap.t * IntSet.t =
      if IntMap.mem tree node then
        ...
        let new_children =
          List.filter children ~f:(fun child -> not (IntSet.mem added child))
        in
        let new_children_no_duplicate =
          IntSet.to_list (IntSet.of_list new_children)
        in
        let tree_with_children_connected =
          IntMap.add_exn tree ~key:node ~data:new_children_no_duplicate
        in
        let added' =
          IntSet.union added (IntSet.of_list new_children_no_duplicate)
        in
        List.fold_left children ~init:(tree_with_children_connected, added')
          ~f:(fun (tree, added) node -> dfs tree added node)
    in
    Tree.of_children_map graph.entry
      (IntMap.to_alist (fst (dfs IntMap.empty IntSet.empty graph.entry)))
  ...
  let%test "spanning tree computation" =
    ...
    let graph =
      of_alist_exn entry exit
        [ (0, [ 4; 2 ]); (4, [ 5; 3 ]); (2, [ 5; 1 ]); (3, [ 1 ]) ]
    in
    let actual = spanning_tree graph in
    (* this is only one of the expected spanning trees *)
    let expected =
      Tree.of_children_map entry
        [ (0, [ 4; 2 ]); (1, []); (2, []); (3, [ 1 ]); (4, [ 5; 3 ]); (5, []) ]
    in
  ...
  let%test "spanning tree computation - reverse example" =
    ...
    let expected =
      Tree.of_children_map exit
        [ (6, [ 2 ]); (2, [ 1; 5 ]); (5, [ 3; 4 ]); (3, []); (4, []); (1, []) ]
    in
    Tree.check_equality ~actual ~expected
  ...
  let%test "spanning tree computation - other reverse example" =
    ...
    let rev_graph =
      reverse
        (of_alist_exn entry exit
           [ (2, [ 3 ]); (3, [ 4; 5 ]); (4, [ 5 ]); (5, [ 6 ]); (6, [ 7 ]) ])
    in
    let actual = spanning_tree rev_graph in
    let expected =
      Tree.of_children_map exit
        [ (2, []); (3, [ 2 ]); (4, []); (5, [ 3; 4 ]); (6, [ 5 ]); (7, [ 6 ]) ]
    in
    Tree.check_equality ~actual ~expected
  ...
  let dominator_tree (graph : 'a t) : Tree.t =
    let rec loop (tree : Tree.t) : [ `Result of Tree.t | `Unreachable of int ] =
      let tree, changed, unreachable =
        List.fold_left graph.nodes ~init:(tree, false, [])
          ~f:(fun (tree, changed, unreachable) node1 ->
            List.fold_left (graph.preds node1)
              ~init:(tree, changed, unreachable)
              ~f:(fun (tree, changed, unreachable) node2 ->
                match Tree.parent tree node1 with
                | Some parent_n1 -> (
                    try
                      let nca =
                        match Tree.nca tree node2 parent_n1 with
                        | Some n -> n
                        | None ->
                            failwith
                              "dominator_tree accessing nodes without common \
                               ancestor"
                      in
                      if parent_n1 <> node2 && nca <> parent_n1 then
                        (Tree.set_parent tree node1 nca, true, unreachable)
                      else (tree, changed, unreachable)
                    with _ -> (tree, changed, node2 :: unreachable))
                | None ->
                    (* This is an unreachable node, part of an infinite loop.
                       Don't visit it and mark it as unreachable *)
                    (tree, changed, node1 :: unreachable)))
      in
      if List.is_empty unreachable then
        if changed then loop tree else `Result tree
      else `Unreachable (List.hd_exn unreachable)
    in
          ...
          outer_loop
            {
              ...
              succs =
                (fun n ->
                  if n = graph.entry then node :: graph.succs n
                  else graph.succs n);
              preds =
                (fun n ->
                  if n = node then graph.entry :: graph.preds n
                  else graph.preds n);
            }
    in
    outer_loop graph
...
let branch_condition (cfg : Spec.t Cfg.t) (block : Spec.t Basic_block.t) :
    Var.t option =
  ...
  | Control c -> (
      ...
      | BrIf _ | BrTable _ | If _ -> (
          ...
          match
            Cfg.state_before_block cfg block.idx (Spec_inference.init_state cfg)
          with
          | Spec.Bottom -> None
...
let cfg_dominator (cfg : Spec.t Cfg.t) : Tree.t =
  Graph.dominator_tree
    (graph_of_cfg (Cfg.without_empty_nodes_with_no_predecessors cfg))
...
module Test = struct
  let%test "dominator tree is correctly computed - wikipedia example" =
    ...
    let exit = 6 in
    let graph =
      Graph.of_alist_exn entry exit
        [ (1, [ 2 ]); (2, [ 3; 4; 6 ]); (3, [ 5 ]); (4, [ 5 ]); (5, [ 2 ]) ]
    in
    let actual : Tree.t = Graph.dominator_tree graph in
    let expected : Tree.t =
      Tree.of_children_map 1
        [ (1, [ 2 ]); (2, [ 3; 4; 5; 6 ]); (3, []); (4, []); (5, []); (6, []) ]
    in
    Tree.check_equality ~actual ~expected

  let%test "post-dominator tree is correctly computed by reversing the order \
            of the graph - wikipedia example" =
    let entry = 1 in
    let exit = 6 in
    let rev_graph =
      Graph.reverse
    ...
    let expected =
      Tree.of_children_map 6
        [ (6, [ 2 ]); (2, [ 1; 5 ]); (5, [ 3; 4 ]); (1, []); (3, []); (4, []) ]
    in
    Tree.check_equality ~actual ~expected

  let%test "post dominator computation should produce expected results on a \
            simple function" =
    let module_ =
    ...
    let expected =
      Tree.of_children_map 7
        [ (2, []); (3, [ 2 ]); (4, []); (5, [ 3; 4 ]); (6, [ 5 ]); (7, [ 6 ]) ]
    in
    Tree.check_equality ~actual ~expected

  let%test_unit "post dominator should work with CFGs that have empty blocks \
  ...
  let%test_unit "post-dominator tree works on a CFG that has one node with two \
  ...
  let%test_unit "post-dominator tree is computed on a CFG that contains an \
                 infinite loop" =
    let module_ =
      Wasm_module.of_string
        "(module\n\
