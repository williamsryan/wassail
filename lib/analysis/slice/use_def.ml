...
module Use = struct
  module T = struct
    type t = {
      label : Instr.Label.t;
          (** The label of the instruction using a variable*)
    ...
    let to_string (use : t) : string =
      Printf.sprintf "use(%s, %s)"
        (Instr.Label.to_string use.label)
  ...
  let to_dot_identifier (cfg : 'a Cfg.t) (use : t) =
    Printf.sprintf "block%d:instr%s"
      (Cfg.find_enclosing_block_exn cfg use.label).idx
      (Instr.Label.to_string use.label)
  ...
  module T = struct
    ...
    type t =
      | Instruction of Instr.Label.t * Var.t
        (* Label of the instruction and variable defined (there can be more than one definition per instruction) *)
      | Entry of Var.t
        (* Only the variable defined (because use-def is intraprocedural, we know the function) *)
      | Constant of
          Prim_value.t (* A constant does not really have a definition *)
    [@@deriving equal, compare]

    let to_string (def : t) : string =
      match def with
      | Instruction (n, v) ->
          Printf.sprintf "idef(%s, %s)" (Instr.Label.to_string n)
            (Var.to_string v)
      | Entry v -> Printf.sprintf "edef(%s)" (Var.to_string v)
      | Constant v -> Printf.sprintf "const(%s)" (Prim_value.to_string v)

    let to_dot_identifier (cfg : 'a Cfg.t) (def : t) : string =
      match def with
      | Instruction (n, _) ->
          Printf.sprintf "block%d:instr%s"
            (Cfg.find_enclosing_block_exn cfg n).idx (Instr.Label.to_string n)
      | Entry v -> Printf.sprintf "entry_var:%s" (Var.to_string v)
      | Constant v -> Printf.sprintf "const:%s" (Prim_value.to_string v)

    let to_dot_label (def : t) : string =
      match def with
...
module UseDefChains = struct
  ...
  module T = struct
    type t = Def.t Use.Map.t [@@deriving compare, equal]

    (** Convert a use-def map to its string representation *)
    let to_string (m : t) : string =
      String.concat ~sep:", "
        (List.map (Use.Map.to_alist m) ~f:(fun (k, v) ->
             Printf.sprintf "%s -> %s" (Use.to_string k) (Def.to_string v)))
  end
  ...
  let add (m : t) (use : Use.t) (def : Def.t) : t =
    ...
    | `Duplicate ->
        failwith
          (Printf.sprintf
             "Cannot have more than one definition for a use in use-def \
              chains, when adding %s ->%s to %s"
             (Use.to_string use) (Def.to_string def) (to_string m))
    | `Ok r -> r
...
let make (cfg : Spec.t Cfg.t) :
    Def.t Var.Map.t * Use.Set.t Var.Map.t * UseDefChains.t =
  ...
  let defs =
    let entry_spec =
      Cfg.state_before_block cfg cfg.entry_block (Spec_inference.init_state cfg)
    in
    ...
    Var.Set.fold vars ~init:defs ~f:(fun defs var ->
        ...
        | `Duplicate ->
            failwith "use_def: more than one entry definition for a variable"
        | `Ok r -> r)
  in
  let all_instrs = Cfg.all_instructions_list cfg in
  (* Add definitions for all constants *)
  let defs =
    let all_vars : Var.Set.t =
      List.fold_left all_instrs ~init:Var.Set.empty ~f:(fun acc instr ->
          Var.Set.union acc
            (Var.Set.union
               (Spec.vars_of (Instr.annotation_before instr))
               (Spec.vars_of (Instr.annotation_after instr))))
    in
    Var.Set.fold all_vars ~init:defs ~f:(fun defs var ->
        match var with
        | Var.Const n -> (
            match Var.Map.add defs ~key:var ~data:(Def.Constant n) with
  ...
  let defs, uses =
    List.fold_left all_instrs ~init:(defs, uses) ~f:(fun (defs, uses) instr ->
        (* Add definitions introduced by this instruction *)
        let defs =
          List.fold_left (Spec_inference.instr_def cfg instr) ~init:defs
            ~f:(fun defs var ->
              match
                Var.Map.add defs ~key:var
                  ~data:(Def.Instruction (Instr.label instr, var))
              with
              | `Duplicate ->
                  failwith
                    (Printf.sprintf
                       "use_def: duplicate define of %s in instruction %s, was \
                        already defined at %s"
                       (Var.to_string var)
                       (Instr.to_string instr ~annot_str:Spec.to_string)
                       (Def.to_string (Var.Map.find_exn defs var)))
              | `Ok r -> r)
        in
        (* Add uses introduced by this instruction *)
        let uses =
          List.fold_left (Spec_inference.instr_use cfg instr) ~init:uses
            ~f:(fun uses var ->
              ...
              Var.Map.update uses var ~f:(function
                ...
                | None -> Use.Set.singleton { label = Instr.label instr; var }))
        in
        (defs, uses))
  in
  (* From the defs and uses map, it is a matter of folding over use to construct the DefUseMap:
     Given a use of v at instruction lab, add key:(lab, v) data:(lookup defs v) *)
  let udchains =
    Var.Map.fold uses ~init:UseDefChains.empty
      ~f:(fun ~key:var ~data:uses map ->
        Use.Set.fold uses ~init:map ~f:(fun map use ->
            UseDefChains.add map use
              (match Var.Map.find defs var with
              | Some v -> v
              | None ->
                  failwith
                    (Printf.sprintf
                       "Use-def chain incorrect: could not find def of a \
                        variable"
                       (* (Var.to_string var) *)))))
  in
  (defs, uses, udchains)
...
let annotate (cfg : Spec.t Cfg.t) : string =
  ...
  String.concat ~sep:"\n"
    (List.map (Use.Map.keys chains) ~f:(fun use ->
...
module Test = struct
  let%test "simplest ud chain" =
    ...
    let expected =
      Use.Map.of_alist_exn
        [
          ( Use.make (lab 2) (Var.Var (lab 0)),
            Def.Instruction (lab 0, Var.Var (lab 0)) );
          ( Use.make (lab 2) (Var.Var (lab 1)),
            Def.Instruction (lab 1, Var.Var (lab 1)) );
          ( Use.make (merge 1) (Var.Var (lab 2)),
            Def.Instruction (lab 2, Var.Var (lab 2)) );
        ]
    in
    UseDefChains.check_equality ~actual ~expected
  ...
  let%test "ud-chain with locals" =
    ...
    let expected =
      Use.Map.of_alist_exn
        [
          (Use.make (lab 0) (Var.Local 0), Def.Entry (Var.Local 0));
  ...
  let%test "use-def with merge blocks" =
    ...
    let expected =
      Use.Map.of_alist_exn
        [
          ( Use.make (lab 1) (Var.Var (lab 0)),
            Def.Instruction (lab 0, Var.Var (lab 0)) );
          ( Use.make (lab 5) (Var.Var (lab 4)),
            Def.Instruction (lab 4, Var.Var (lab 4)) );
          ( Use.make (lab 5) (Var.Merge (4, 1)),
            Def.Instruction (merge 4, Var.Merge (4, 1)) );
          ( Use.make (merge 4) (Var.Var (lab 2)),
            Def.Instruction (lab 2, Var.Var (lab 2)) );
          ( Use.make (merge 4) (Var.Var (lab 3)),
            Def.Instruction (lab 3, Var.Var (lab 3)) );
          ( Use.make (merge 6) (Var.Var (lab 5)),
            Def.Instruction (lab 5, Var.Var (lab 5)) );
        ]
    in
    UseDefChains.check_equality ~actual ~expected
