...
module InSlice = struct
  module T = struct
    type t = {
      label : Instr.Label.t;
          (** The label of the instruction that should be added to the slice *)
      reason : Var.t option;
          (** The corresponding var that make this instruction part of the slice, if there is one *)
    }
    ...
    let to_string (t : t) : string =
      ...
      | Some var ->
          Printf.sprintf "%s(%s)"
            (Instr.Label.to_string t.label)
            (Var.to_string var)

    let make (label : Instr.Label.t) (var : Var.t option)
        (instructions : 'a Instr.t Instr.Label.Map.t) =
      {
        label;
        reason =
          (match Cfg.find_instr instructions label with
...
let preanalysis (cfg : Spec.t Cfg.t)
    (cfg_instructions : Spec.t Instr.t Instr.Label.Map.t) : preanalysis_results
    =
  ...
  let global_set_instructions =
    InSlice.Set.of_list
      (List.map
         ~f:(fun label -> InSlice.{ label; reason = None })
         (Instr.Label.Map.keys
            (Instr.Label.Map.filter cfg_instructions ~f:(function
...
(** Identify instructions to keep in a backwards slice on `cfg`, using the
    ...
    labels. *)
let instructions_to_keep (cfg : Spec.t Cfg.t)
    (cfg_instructions : Spec.t Instr.t Instr.Label.Map.t)
    (preanalysis : preanalysis_results) (criteria : Instr.Label.Set.t) :
    Instr.Label.Set.t
    * (Time_float.Span.t
      * Time_float.Span.t
      * Time_float.Span.t
      ...
      * Time_float.Span.t) =
  Log.info
    (Printf.sprintf "Slicing with criteria %s"
       (Instr.Label.Set.to_string criteria));
  let t0 = Time_float.now () in
  let rec loop (worklist : InSlice.Set.t) (slice : Instr.Label.Set.t)
      (visited : InSlice.Set.t) : Instr.Label.Set.t =
    ...
    | Some slicepart ->
        Log.info
          (Printf.sprintf "Looking at instruction %s"
             (InSlice.to_string slicepart));
        (* Add instr to the current slice *)
        ...
        let uses =
          ...
          | None ->
              failwith
                "Unsupported in slicing: cannot find an instruction. It \
                 probably is part of unreachable code."
          | Some instr ->
              Spec_inference.instr_use cfg ?var:slicepart.reason instr
        in
        (* For use in instr_uses(instr) *)
        let worklist' =
          List.fold_left uses ~init:worklist ~f:(fun w use ->
              (* Get the definition corresponding to the current use *)
              let def =
                Use_def.UseDefChains.get preanalysis.data_dependencies
                  (Use_def.Use.make slicepart.label use)
              in
              (* For def in usedef(use): if def contains an instruction, add def.instr to W *)
              let data_deps : InSlice.Set.t =
                match def with
                | Use_def.Def.Instruction (instr', var) ->
                    Log.info
                      (Printf.sprintf
                         "Instruction %s (%s) is part of the slice due to its \
                          data dependence on %s"
                         (Instr.Label.to_string instr')
                         (Instr.to_string
                            (Instr.Label.Map.find_exn cfg_instructions instr'))
                         (Var.to_string var));
                    InSlice.Set.singleton
                      (InSlice.make instr' (Some var) cfg_instructions)
                | Use_def.Def.Entry _ -> InSlice.Set.empty
                | Use_def.Def.Constant _ -> InSlice.Set.empty
              in
              InSlice.Set.union w data_deps)
        in
        (* Add all control dependencies of instr to W *)
        let control_deps : InSlice.Set.t =
          match
            Instr.Label.Map.find preanalysis.control_dependencies
              slicepart.label
          with
          | None -> InSlice.Set.empty
          | Some deps ->
              InSlice.Set.of_list
                (List.map (Instr.Label.Set.to_list deps) ~f:(fun label ->
                     Log.info
                       (Printf.sprintf
                          "Instruction %s (%s) is part of the slice due to \
                           control dependences"
                          (Instr.Label.to_string label)
                          (Instr.to_string
                             (Instr.Label.Map.find_exn cfg_instructions label)));
            ...
            (InSlice.Set.of_list
               (List.map
                  ...
                  (Instr.Label.Set.to_list
                     (Memory_deps.deps_for preanalysis.mem_dependencies
                        slicepart.label))))
        in
        loop (InSlice.Set.remove worklist''' slicepart) slice' visited'
  in
  let agrawal (slice : Instr.Label.Set.t) : Instr.Label.Set.t =
    ...
    Instr.Label.Set.fold
      (Instr.Label.Set.of_list (Instr.Label.Map.keys cfg_instructions))
      ~init:slice
      ~f:(fun slice label ->
        ...
        | Control { instr = Br _; _ } -> (
            match
              Instr.Label.Map.find preanalysis.control_dependencies label
            with
            | Some instrs -> (
                match
                  Instr.Label.Set.find_map instrs ~f:(fun i ->
                      if Instr.Label.Set.mem slice i then Some i else None)
                with
                | Some label' ->
                    Log.info
                      (Printf.sprintf
                         "Agrawal tells us to add %s to the slice because \
                          there is a dependency to %s\n"
                         (Instr.Label.to_string label)
                         (Instr.Label.to_string label'));
  ...
  let initial_worklist =
    InSlice.Set.union preanalysis.global_set_instructions
      (InSlice.Set.of_list
         (List.map (Instr.Label.Set.to_list criteria) ~f:(fun criterion ->
              InSlice.{ label = criterion; reason = None })))
  in
  let initial_slice = Instr.Label.Set.empty in
  let slice =
    Instr.Label.Set.filter
      (agrawal (loop initial_worklist initial_slice InSlice.Set.empty))
      ~f:(fun lab ->
        match lab.section with
        | MergeInFunction _ ->
  ...
  let t1 = Time_float.now () in
  ( slice,
    ( preanalysis.control_time,
      preanalysis.data_time,
      preanalysis.mem_time,
      preanalysis.global_time,
      Time_float.diff t1 t0 ) )

type instr_type_element = T of Type.t | Any of string [@@deriving equal]

let instr_type_element_to_string t =
  match t with T t -> Type.to_string t | Any _ -> "any"

let type_of_data (i : (Instr.data, 'a) Instr.labelled) (cfg : 'a Cfg.t)
  ...
  | None ->
      Log.warn
        (Printf.sprintf "instruction is unreachable: %s"
  ...
  | Some _ -> (
      ...
      | Convert op ->
          ([ Any "a" ], [ T op.typ ])
          (* TODO: it should really be a specific type rather than any *)
...
let type_of_control (i : ('a Instr.control, 'a) Instr.labelled)
    (_cfg : unit Cfg.t) (instructions_map : Spec.t Instr.t Instr.Label.Map.t) :
    instr_type_element list * instr_type_element list =
  let vstack_before =
    match Cfg.find_instr instructions_map i.label with
    | None -> None
  ...
  | _, Some vstack_before -> (
      ...
      match i.instr with
      | Call (_, (in_type, out_type), _) ->
          ( List.map in_type ~f:(fun t -> T t),
            List.map out_type ~f:(fun t -> T t) )
      | CallIndirect (_, _, (in_type, out_type), _) ->
          ( List.map in_type ~f:(fun t -> T t) @ [ T Type.I32 ],
            List.map out_type ~f:(fun t -> T t) )
      | If (bt, _, _, _) ->
          (* the net effect of the head, which drops the first element of the stack *)
          ([ T Type.I32 ], match bt with Some t -> [ T t ] | None -> [])
      | Br _ ->
      ...
      | Block _ | Loop _ ->
          ...
          (* because we have the block/loop instructions handled in the first match *)
      )

(** Construct a dummy list of instruction that has the given type *)
let dummy_instrs (t : instr_type_element list * instr_type_element list)
    (next_label : unit -> int) : (Instr.data, unit) Instr.labelled list =
  let dummy_label () : Instr.Label.t =
    { section = Instr.Label.Dummy; id = next_label () }
  in
  (* before anything, we remove parts of types that won't be needed, e.g., [i32] -> [i32] can be replaced by [] -> [] *)
  let rec loop (l1 : instr_type_element list) (l2 : instr_type_element list)
      (n : int) : int =
    match (l1, l2) with
    | Any _ :: t1, _ :: t2 | _ :: t1, Any _ :: t2 -> loop t1 t2 (n + 1)
  ...
  let input =
    List.map input ~f:(fun _ ->
        {
          Instr.instr = Instr.Drop;
          label = dummy_label ();
          line_number = -1;
          annotation_before = ();
          annotation_after = ();
  ...
  in
  let push (v : Prim_value.t) =
    {
      Instr.instr = Instr.Const v;
      label = dummy_label ();
      line_number = -1;
      annotation_before = ();
      annotation_after = ();
...
let instrs_type (instrs : unit Instr.t list) (cfg : 'a Cfg.t)
    (instructions_map : Spec.t Instr.t Instr.Label.Map.t) :
    instr_type_element list * instr_type_element list =
  let input, output =
    List.fold_left instrs ~init:([], [])
      ~f:(fun (initial_stack, current_stack) instr ->
        let i, o = type_of instr cfg instructions_map in
        let initial_stack, current_stack =
          List.fold_left i ~init:(initial_stack, current_stack)
            ~f:(fun (initial_stack, current_stack) t ->
              match current_stack with
              | _ :: rest ->
                  (* We don't check that the types match, as we assume they will *)
                  (initial_stack, rest)
              | [] -> (t :: initial_stack, current_stack))
        in
        (initial_stack, List.rev o @ current_stack))
  in
...
let replace_with_equivalent_instructions (instrs : unit Instr.t list)
    (cfg : 'a Cfg.t) (instructions_map : Spec.t Instr.t Instr.Label.Map.t) :
    unit Instr.t list =
  if List.is_empty instrs then instrs
  else
    ...
    in
    Log.info
      (Printf.sprintf "Replacing instructions %s of type %s -> %s with %s"
         (String.concat ~sep:"," (List.map ~f:Instr.to_string instrs))
         (String.concat ~sep:","
            (List.map ~f:instr_type_element_to_string (fst t)))
         (String.concat ~sep:","
            (List.map ~f:instr_type_element_to_string (snd t)))
...
let body_can_be_removed (body : unit Instr.t list) : bool =
  ...
  List.for_all body ~f:(fun instr ->
      Instr.Label.equal_section (Instr.label instr).section Instr.Label.Dummy)

let rec slice (cfg : 'a Cfg.t)
    (cfg_instructions : Spec.t Instr.t Instr.Label.Map.t)
    (original_instructions : unit Instr.t list)
    (instructions_to_keep : Instr.Label.Set.t) : unit Instr.t list =
  let rec loop (instrs : unit Instr.t list) (to_remove_rev : unit Instr.t list)
      : unit Instr.t list =
    match instrs with
    | [] ->
        replace_with_equivalent_instructions (List.rev to_remove_rev) cfg
          cfg_instructions
    | (Control ({ instr = Block (bt, arity, body); _ } as instr) as entire_instr)
      :: rest ->
        (* if (fst arity) > 0 || (snd arity) > 0 then failwith "Unsupported: block with arity greater than 0"; *)
        let sliced_body =
          slice cfg cfg_instructions body instructions_to_keep
        in
        ...
        if body_can_be_removed sliced_body then
          ...
          @ [
              Instr.Control
                { instr with instr = Block (bt, arity, sliced_body) };
            ]
          @ loop rest []
    | (Control ({ instr = Loop (bt, arity, body); _ } as instr) as entire_instr)
      :: rest ->
        ...
        if body_can_be_removed sliced_body then
          ...
          replace_with_equivalent_instructions (List.rev to_remove_rev) cfg
            cfg_instructions
          @ [
              Instr.Control { instr with instr = Loop (bt, arity, sliced_body) };
            ]
          @ loop rest []
    | (Control ({ instr = If (bt, arity, then_, else_); _ } as instr) as
       entire_instr)
      :: rest ->
        (* if (fst arity) > 0 || (snd arity) > 0 then failwith "Unsupported: if with arity greater than 0"; *)
        let sliced_then =
          slice cfg cfg_instructions then_ instructions_to_keep
        in
        let sliced_else =
          slice cfg cfg_instructions else_ instructions_to_keep
        in
        if body_can_be_removed sliced_then && body_can_be_removed sliced_else
        then loop rest (entire_instr :: to_remove_rev)
        else
          replace_with_equivalent_instructions (List.rev to_remove_rev) cfg
            cfg_instructions
          @ [
              Instr.Control
                { instr with instr = If (bt, arity, sliced_then, sliced_else) };
            ]
          @ loop rest []
    | instr :: rest
      when Instr.Label.Set.mem instructions_to_keep (Instr.label instr) ->
        replace_with_equivalent_instructions (List.rev to_remove_rev) cfg
          cfg_instructions
        @ [ instr ] @ loop rest []
    | instr :: rest -> loop rest (instr :: to_remove_rev)
  in
  loop original_instructions []

let slice_to_funcinst (cfg : Spec.t Cfg.t)
    (cfg_instructions : Spec.t Instr.t Instr.Label.Map.t)
    ?instrs:(instructions_in_slice : Instr.Label.Set.t option = None)
    (slicing_criteria : Instr.Label.Set.t) : Func_inst.t =
  let instructions_in_slice =
    match instructions_in_slice with
    | Some instrs -> instrs
    | None ->
        Log.info "Computing instructions part of the slice";
        let instrs, _ =
          instructions_to_keep cfg cfg_instructions
            (preanalysis cfg cfg_instructions)
            slicing_criteria
        in
        instrs
  in
  ...
  let instructions =
    slice unit_cfg cfg_instructions (Cfg.body unit_cfg) instructions_in_slice
  in
  {
    idx = cfg.idx;
    name = Some cfg.name;
...
let find_call_indirect_instructions (cfg : Spec.t Cfg.t) : Instr.Label.t list =
  List.filter_map (Cfg.all_instructions_list cfg) ~f:(fun instr ->
      match instr with
...
module Test = struct
  ...
  let build_cfg ?(fidx : int32 = 0l) (program : string) :
      Wasm_module.t * Spec.t Cfg.t =
  ...
  let%test "simple slicing - first slicing criterion, only const" =
    ...
    let actual, _ =
      instructions_to_keep cfg all_instrs
        (preanalysis cfg all_instrs)
        (Instr.Label.Set.singleton (lab 2))
    in
  ...
  let%test "simple slicing - second slicing criterion, with locals" =
    ...
    let actual, _ =
      instructions_to_keep cfg all_instrs
        (preanalysis cfg all_instrs)
        (Instr.Label.Set.singleton (lab 6))
    in
  ...
  let%test "slicing with block and br_if" =
    ...
    let _, cfg =
      build_cfg
        ...
        \      br_if 0     ;; Instr 2 -- control dependency of the slicing \
         criterion\n\
    ...
    let actual, _ =
      instructions_to_keep cfg all_instrs
        (preanalysis cfg all_instrs)
        (Instr.Label.Set.singleton (lab 3))
    in
  ...
  let%test "slicing with block and br_if -- second slicing criterion" =
    ...
    let _, cfg =
      build_cfg
        ...
        \      drop        ;; Instr 4 -- slicing criterion, has a data dep on \
         instr 3\n\
        \    end\n\
        \    local.get 0)  ;; Instr 5\n\
        \  )"
    ...
    let actual, _ =
      instructions_to_keep cfg all_instrs
        (preanalysis cfg all_instrs)
        (Instr.Label.Set.singleton (lab 4))
    in
  ...
  let%test "slicing with merge blocks" =
    ...
    let actual, _ =
      instructions_to_keep cfg all_instrs
        (preanalysis cfg all_instrs)
        (Instr.Label.Set.singleton (lab 9))
    in
    (* Merge blocks do not need to be in the slice *)
    let expected =
      Instr.Label.Set.of_list [ lab 0; lab 1; lab 2; lab 3; lab 8; lab 9 ]
    in
    Instr.Label.Set.check_equality ~actual ~expected

  let%test_unit "slicing with a block containing a single drop should produce \
                 a valid slice" =
    Spec_inference.propagate_globals := false;
    Spec_inference.propagate_locals := false;
    Spec_inference.use_const := false;
    ...
    in
    let _funcinst =
      slice_to_funcinst cfg (Cfg.all_instructions cfg)
        (Instr.Label.Set.singleton (lab 8))
    in
    ()
  ...
  let check_slice original sliced fidx criterion =
    ...
    let expected =
      (slice_to_funcinst cfg (Cfg.all_instructions cfg)
         (Instr.Label.Set.singleton (lab ~fidx criterion)))
        .code
        .body
    in
    ...
    List.equal
      (fun x y ->
        if
          Instr.equal
            (fun () () -> true)
            (Instr.drop_labels x) (Instr.drop_labels y)
        then true
        else (
  ...
  let%test "slice with merge block should not contain non-relevant \
  ...
  let%test "slice on the example from Agrawal 1994 (Fig. 3) should be correct" =
    let original =
      ...
      \        br_if 1 ;; goto end of block 0 if eof() --> Instr 3 should be \
       part of the slice\n\
      \        block ;; block 1\n\
      \          block ;; block 2\n\
      \            call 1 ;; read() --> Inst r6 should be part of the slice\n\
      \            local.tee 2 ;; x = read()\n\
      \            br_if 0 ;; jump to end of block 2 (L8) if x != 0 (was: x > \
       0) --> Instr 8 should be part of the slice\n\
      \            local.get 2\n\
      \            call 2 ;; f(x)\n\
      \            local.get 0\n\
      \            i32.add ;; sum + f(x)\n\
      ...
      \            local.set 1 ;; positives = positives + 1 --> should be part \
       of the slice\n\
      \            local.get 2\n\
      \            br_if 0 ;; jump to end of block 3 (L12) if x != 0 (was: x%2 \
       != 0)\n\
      \            local.get 2\n\
      \            call 2 ;; f(x) (was: f2(x))\n\
      \            local.get 0\n\
      ...
      \        br 0 ;; jump to beginning of loop 0 (L3) ;; --> Instr 33 should \
       be part of the slice (with Agrawal's algorithm, not the conventional \
       one!)\n\
      \      end ;; end of loop 0\n\
      ...
      \    ;; The following instruction is the slicing criterion, i.e., \
       instruction number 38\n\
      \    call 2 ;; f(positives) (was: write(positives)) --> Instr 38 should \
    ...
    let sliced =
      ...
      \  (func (;3;) (type 0)\n\
      \    (local i32 i32 i32)\n\
      \    ;; If a line is annotated with Agrawal, it means it is part of \
       Agrawal's original solution\n\
      \    ;; If a line is annotated with superfluous, it means it is part of \
       our slice because we don't assume functions to be pure, while Agrawal \
       does. Hence, our solution is an over-approximation of Agrawal's, but it \
       is still correct.\n\
      \    block\n\
      \      loop\n\
      \        call 0 ;; eof(), Agrawal\n\
  ...
  let%test "slice on the example from Agrawal 1994 (Fig. 5) should be correct" =
    let original =
      ...
      \        br_if 1 ;; goto end of block 0 if !eof() --> Instr 5, part of \
       the slice\n\
      ...
      \          i32.add ;; sum + f(x)\n\
      \          local.set 0 ;; sum = sum + f(x)\n\
      \          br 1 ;; jump to the beggining of loop 0 (L3) ;; --> Instr 14, \
       part of the slice (with Agrawal's additions)\n\
      \        end\n\
      \        local.get 1 ;; --> part of the slice\n\
      \        i32.const 1 ;; --> part of the slice\n\
      \        i32.add     ;; --> part of the slice\n\
      \        local.set 1 ;; positives = positives + 1 --> Instr 18,  part of \
       the slice\n\
      \        local.get 2\n\
      \        if ;; if (x != 0) (was: x%2 != 0)\n\
      \          local.get 2\n\
      ...
      \    call 2 ;; f(positives) (was: write(positives)) --> Instr 37, part \
       of the slice\n\
      \    drop\n\
      \    ))"
    in
  ...
  let%test_unit "slicing function 14 of trmm" =
    let module_ =
      Wasm_module.of_file "../../../benchmarks/polybench-clang/trmm.wat"
    in
    Spec_inference.propagate_globals := false;
    Spec_inference.propagate_locals := false;
    Spec_inference.use_const := false;
    ...
    List.iter (find_call_indirect_instructions cfg) ~f:(fun instr_idx ->
        ...
        let funcinst =
          slice_to_funcinst cfg (Cfg.all_instructions cfg)
            (Instr.Label.Set.singleton instr_idx)
        in
  ...
  let%test "slicing of SCAM mug example (variant 1) should produce the full \
            program as the slice" =
    let original =
      "(module\n\
      \  (type (;0;) (func))\n\
      \  (type (;1;) (func (result i32)))\n\
  ...
  let%test "slicing of SCAM mug example (variant 2) should produce the full \
            program as the slice" =
    let original =
      "(module\n\
      \  (type (;0;) (func))\n\
  ...
  let%test "slicing of Montréal boat example should produce the full program \
            as the slice" =
    let original =
      "(module\n\
      \  (type (;0;) (func))\n\
  ...
  let%test "slicing of word count example (slicing criterion 1) should produce \
            the expected slice" =
    let slice =
      "(module\n\
       (type (;0;) (func))\n\
  ...
  let%test "slicing of word count example (slicing criterion 2) should produce \
            the expected slice" =
    let slice =
      "(module\n\
       (type (;0;) (func))\n\
  ...
  let%test "slicing of word count example (slicing criterion 3) should produce \
            the expected slice" =
    let slice =
      "(module\n\
       (type (;0;) (func))\n\
  ...
  let%test "slicing of word count example (slicing criterion 4) should produce \
            the expected slice" =
    let slice =
      "(module\n\
       (type (;0;) (func))\n\
  ...
  let%test "slicing of word count example (slicing criterion 5) should produce \
            the expected slice" =
    let slice =
      "(module\n\
       (type (;0;) (func))\n\
  ...
  let%test "slicing blocks with results and br" =
    let original =
      ...
      \      if\n\
      \        i32.const 1\n\
      \        ;; The difficulty lies in removing this instruction, which is \
       not considered part of the slice.\n\
      \        ;; However, its effect on the stack (-1) depends on the current \
       block arity (0) and the target block arity (1)\n\
      \        br 1\n\
      \      end\n\
      \      i32.const 2\n\
      \    end\n\
