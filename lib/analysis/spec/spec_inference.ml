...
module Spec_inference = struct
  ...
  let drop (n : int) (vstack : Var.t list) =
    ...
    else List.drop vstack n

  let top (l : Var.t list) =
  ...
  let set (n : Int32.t) (l : Var.t list) (v : Var.t) =
    List.mapi l ~f:(fun i v' -> if i = Int32.to_int_exn n then v else v')

  (*---- State ----*)
  type state = Spec.t [@@deriving compare, equal]
  ...
  let init_state (cfg : 'a Cfg.t) : state =
    Spec.NotBottom
      {
        vstack = [];
        (* the vstack is initially empty *)
        locals =
          (if !use_const then
             List.mapi cfg.arg_types ~f:(fun i _ -> Var.Local i)
             @ List.map cfg.local_types ~f:(fun _ ->
                   Var.Const (Prim_value.I32 0l))
           else
             List.mapi (cfg.arg_types @ cfg.local_types) ~f:(fun i _ ->
                 Var.Local i));
  ...
  let rec compute_stack_size_at_entry (cfg : annot_expected Cfg.t)
      (label : Instr.Label.t) (state : SpecWithoutBottom.t) :
      SpecWithoutBottom.t =
    match Cfg.find_enclosing_block cfg label with
    | None ->
        (* no enclosing block, the stack is initally empty at the beginning of the function, we ignore that *)
        state
    | Some block_label -> (
        match Instr.Label.Map.find state.stack_size_at_entry block_label with
        | None ->
            ...
            let state = compute_stack_size_at_entry cfg block_label state in
            {
              state with
              stack_size_at_entry =
                Instr.Label.Map.set state.stack_size_at_entry ~key:block_label
                  ~data:size;
            }
        | Some _ ->
  ...
  let data_instr_transfer (_module_ : Wasm_module.t)
      (cfg : annot_expected Cfg.t) (i : annot_expected Instr.labelled_data) :
      state -> state =
    Spec.lift (function state ->
        (let ret = Var.Var i.label in
         ...
         | LocalGet l ->
             {
               state with
               vstack =
                 (if !propagate_locals then get l state.locals else ret)
                 :: state.vstack;
             }
         | LocalSet l ->
             {
               state with
               vstack = drop 1 state.vstack;
               locals =
                 set l state.locals
                   (if !propagate_locals then top state.vstack else ret);
             }
         | LocalTee l ->
             {
               state with
         ...
         | GlobalGet g ->
             {
               state with
               vstack =
                 (if !propagate_globals then get g state.globals else ret)
                 :: state.vstack;
             }
         | GlobalSet g ->
             {
               ...
               globals =
                 set g state.globals
                   (if !propagate_globals then top state.vstack else ret);
             }
         | Const n ->
             {
               state with
               vstack =
                 (if !use_const then Var.Const n else ret) :: state.vstack;
             }
         ...
         | Store { offset; _ } ->
             ...
             {
               state with
               vstack = drop 2 state.vstack;
               memory =
                 Var.OffsetMap.update state.memory (addr, offset) ~f:(fun _ ->
                     value);
             }
  ...
  let control_instr_transfer (_module_ : Wasm_module.t) _summaries
      (cfg : 'a Cfg.t) (i : ('a Instr.control, 'a) Instr.labelled) :
      state -> [ `Simple of state | `Branch of state * state ] =
    Spec.wrap ~default:(`Simple bottom) (function state ->
        (let ret = Var.Var i.label in
         let state = compute_stack_size_at_entry cfg i.label state in
         let get_block_return_stack_size n =
           let stack_size_at_entry =
             match Cfg.find_enclosing_block cfg i.label with
             | None -> 0 (* the stack is initially empty *)
             | Some block_label -> (
                 match
                   Instr.Label.Map.find state.stack_size_at_entry block_label
                 with
                 | Some size -> size
                 | None ->
                     failwith
                       "Spec inference: no stack size computed at entry of \
                        block")
           in
           let out_arity =
             match Cfg.find_nth_parent_block cfg i.label n with
             | Some block_label ->
             ...
             | None -> List.length cfg.return_types
           in
           stack_size_at_entry + out_arity
         in
         match i.instr with
               ...
               (NotBottom
                  {
                    state with
                    vstack =
                      (if arity_out = 1 then [ ret ] else [])
                      @ drop arity_in state.vstack;
                  })
         ...
         | Br n ->
             `Simple
               (NotBottom
                  {
                    state with
                    vstack = take state.vstack (get_block_return_stack_size n);
                  })
         | BrIf n ->
             let state' =
               NotBottom
                 {
                   state with
                   vstack =
                     take (drop 1 state.vstack) (get_block_return_stack_size n);
                 }
             in
             `Branch (state', state')
         | If _ ->
         ...
         | BrTable (ns, n) ->
             ...
             `Simple
               (NotBottom
                  { state with vstack = take (drop 1 state.vstack) arity })
         | Return ->
             `Simple
               (NotBottom
                  {
                    state with
                    vstack = take state.vstack (List.length cfg.return_types);
                  })
         | Unreachable ->
         ...
         | _ ->
             failwith
               (Printf.sprintf "Unsupported control instruction: %s"
                  (Instr.control_to_short_string i.instr))))

  let merge (_module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (block : annot_expected Basic_block.t) (states : state list) : state =
      ...
      if cfg.exit_block = block.idx then
        {
          s with
          vstack =
            List.mapi s.vstack ~f:(fun i v -> if i = 0 then Var.Return else v);
        }
      else s
    in
    ...
    | Control { instr = Merge; _ } ->
        ...
        (Spec.lift rename_exit)
          ...
          | _ -> (
              ...
              match
                List.filter states ~f:(fun s -> not (equal_state s bot))
              with
              | [] ->
              ...
              | state :: states ->
                  ...
                  let with_holes =
                    List.fold_left states ~init:state ~f:(fun acc s ->
                        match (acc, s) with
                        | Bottom, Bottom -> Bottom
                        | NotBottom s, Bottom | Bottom, NotBottom s ->
                            NotBottom s
                        | NotBottom acc, NotBottom s ->
                            ...
                            in
                            if List.length acc.vstack <> List.length s.vstack
                            then
                              failwith
                                "unsupported in spec_inference: incompatible \
                                 stack lengths (probably due to mismatches in \
                                 br_table branches)";
                            assert (
                              List.length acc.locals = List.length s.locals);
                            assert (
                              List.length acc.globals = List.length s.globals);
                            NotBottom
                              {
                                vstack = List.map2_exn acc.vstack s.vstack ~f;
                                locals = List.map2_exn acc.locals s.locals ~f;
                                globals = List.map2_exn acc.globals s.globals ~f;
                                memory =
                                  Var.OffsetMap.merge acc.memory s.memory
                                    ~f:(fun ~key:_ v ->
                                      match v with
                                      ...
                                      | `Left _v | `Right _v ->
                                          ...
                                          None);
                                stack_size_at_entry =
                                  Instr.Label.Map.merge acc.stack_size_at_entry
                                    s.stack_size_at_entry
                                    ~f:(fun ~key:_ presence ->
                                      match presence with
                                      | `Both (a, b) ->
                                          if a <> b then
                                            failwith
                                              "Cannot merge due to different \
                                               stack sizes"
                                          else Some a
                  ...
                  let plug_holes = function
                    ...
                    | v -> (* no hole, keep the variable *) v
                  in
                  lift
                    (fun s ->
                      {
                        vstack = List.map s.vstack ~f:plug_holes;
    ...
    | _ -> (
        ...
        | _ ->
            failwith
              (Printf.sprintf "Invalid block with multiple input states: %d"
                 block.idx))

  let merge_flows (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (block : annot_expected Basic_block.t) (states : (int * state) list) :
      state =
...
let new_merge_variables (cfg : t Cfg.t) (merge_block : t Basic_block.t) :
    (Var.t * Var.t) list =
  (* The predecessors of merge_block *)
  let preds = Cfg.predecessors cfg merge_block.idx in
  let state_after =
    Cfg.state_after_block cfg merge_block.idx (init_state cfg)
  in
  List.fold_left preds ~init:[] ~f:(fun acc pred_idx ->
      ...
      if Spec_inference.equal state_before Spec_inference.bottom then
        (* Ignore bottom state *)
        acc
      else extract_different_vars state_before state_after @ acc)
...
let instr_def (cfg : t Cfg.t) (instr : t Instr.t) : Var.t list =
  let defs =
    ...
    | Instr.Data i -> (
        let state_after =
          match i.annotation_after with
          | Bottom ->
              failwith "bottom annotation, this an unreachable instruction"
          | NotBottom s -> s
        in
        let top_n n = take state_after.vstack n in
        match i.instr with
        | Nop | Drop | MemoryCopy | MemoryFill | MemoryInit _ -> []
        | Select _ | MemorySize | Unary _ | Binary _ | Compare _ | Test _
        | Convert _ | Const _ | MemoryGrow | RefIsNull | RefNull _ | RefFunc _
          ->
            top_n 1
        | LocalGet _ -> if !propagate_locals then [] else top_n 1
        | GlobalGet _ -> if !propagate_globals then [] else top_n 1
        | LocalSet l | LocalTee l ->
            if !propagate_locals then [] else [ get_nth state_after.locals l ]
        | GlobalSet g ->
            if !propagate_globals then [] else [ get_nth state_after.globals g ]
        | Load _ -> top_n 1
        | Store _ -> [])
    | Instr.Control i -> (
        let top_n n =
          ...
          | NotBottom s -> take s.vstack n
        in
        match i.instr with
        | Block _ | Loop _ ->
            []
            (* we handle instruction individually rather than through their block *)
        | If _ ->
            []
            (* We could say that if defines its "resulting" value, but that will be handled by the merge node *)
  ...
  List.filter defs ~f:(function
    | Var.Const _ ->
        false
        (* constants are not variables that can be defined (otherwise definitions are not unique anymore) *)
    | Var.Local _ ->
        false
        (* locals don't have a definition point (they are part of the entry state) *)
 ...
 *)
let instr_use (cfg : t Cfg.t) ?(var : Var.t option) (instr : t Instr.t) :
    Var.t list =
  match instr with
  | Instr.Data i -> (
      let top_n n =
        match i.annotation_before with
        | Bottom -> failwith "bottom annotation"
        | NotBottom s -> take s.vstack n
      in
      match i.instr with
      ...
      | LocalGet l ->
          let locals =
            match i.annotation_before with
            | Bottom -> failwith "bottom annotation"
            | NotBottom s -> s.locals
          in
          [ get_nth locals l ]
          (* use local l *)
      | LocalSet _ | LocalTee _ ->
          top_n 1 (* use top of the stack to define local *)
      | GlobalGet g ->
          let globals =
            match i.annotation_before with
            | Bottom -> failwith "bottom annotation"
            | NotBottom s -> s.globals
          in
          [ get_nth globals g ]
          (* use global g *)
      | GlobalSet _ -> top_n 1
  ...
  | Instr.Control i -> (
      ...
      | Call ((arity_in, _), _, _) ->
          top_n arity_in (* uses the n arguments from the stack *)
      | CallIndirect (_, (arity_in, _), _, _) ->
          top_n (arity_in + 1)
          (* + 1 because we need to pop the index that will refer to the called function, on top of the arguments *)
      | BrIf _ | BrTable _ -> top_n 1
      | Merge -> (
          (* Merge instruction uses the variables it redefines *)
          let block = Cfg.find_enclosing_block_exn cfg (Instr.label instr) in
          match var with
          | None -> List.map (new_merge_variables cfg block) ~f:fst
          | Some v ->
              List.filter_map (new_merge_variables cfg block)
                ~f:(fun (old, new_) ->
                  if Var.equal new_ v then Some old else None))
      | Return -> top_n (List.length cfg.return_types)
      | Br _ | Unreachable -> [])
