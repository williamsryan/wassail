...
module T = struct
  type t =
    ...
    | Merge of int * int
      (* merge variable (n, m) meaning from nth function, mth merge variable *)
    | Return (* return variable *)
    | Hole (* only temporary holes meant to be replaced by merge variables *)
    | Const of Prim_value.t
    | Other of string
      (* sometimes it is useful to add new variables, depending on the client analysis *)
  [@@deriving sexp, compare, equal]

  type with_offset = t * int [@@deriving sexp, compare, equal]
  ...
  let list_to_string (vs : t list) : string =
    String.concat ~sep:", " (List.map vs ~f:to_string)
end
...
module OffsetMap = struct
  ...
  include Map.Make (struct
    type t = with_offset [@@deriving sexp, compare, equal]
  end)

  let map_vars (m : T.t t) ~(f : T.t -> T.t) : T.t t =
    of_alist_exn
      (List.map (Map.to_alist m) ~f:(fun ((k, offset), v) ->
           ((f k, offset), f v)))
end
...
module Set = struct
  ...
  let of_option (v : T.t option) : t =
    match v with Some v -> singleton v | None -> empty
end
