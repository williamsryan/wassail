...
module SpecWithoutBottom = struct
  ...
  let to_string (s : t) : string =
    Printf.sprintf
      ...
      (String.concat ~sep:", "
         (List.map (Var.OffsetMap.to_alist s.memory) ~f:(fun ((k, offset), v) ->
              Printf.sprintf "%s+%d: %s" (Var.to_string k) offset
                (Var.to_string v))))
      (String.concat ~sep:", "
         (List.map
            ~f:(fun (k, v) ->
              Printf.sprintf "%s: %d" (Instr.Label.to_string k) v)
            (Instr.Label.Map.to_alist s.stack_size_at_entry)))

  let to_dot_string (s : t) : string =
    String.concat ~sep:"|"
      (List.map s.vstack ~f:(fun v ->
  ...
  let map_vars (s : t) ~(f : Var.t -> Var.t) : t =
    {
      vstack = List.map s.vstack ~f;
      locals = List.map s.locals ~f;
      globals = List.map s.globals ~f;
      memory = Var.OffsetMap.map_vars s.memory ~f;
  ...
  let memvars (s : t) : Var.t list =
    List.concat
      (List.map (Var.OffsetMap.to_alist s.memory) ~f:(fun ((k, _offset), v) ->
           [ k; v ]))
  ...
  let extract_different_vars (s1 : t) (s2 : t) : (Var.t * Var.t) list =
    let f (l1 : Var.t list) (l2 : Var.t list) : (Var.t * Var.t) list =
      ...
      List.filter_map
        (List.map2_exn l1 l2 ~f:(fun v1 v2 -> (v1, v2, Var.equal v1 v2)))
        ~f:(fun (v1, v2, eq) -> if not eq then Some (v1, v2) else None)
    in
    let fvstack (l1 : Var.t list) (l2 : Var.t list) : (Var.t * Var.t) list =
      ...
      f (List.take l1 min_size) (List.take l2 min_size)
    in
    let fmap (m1 : Var.t Var.OffsetMap.t) (m2 : Var.t Var.OffsetMap.t) :
        (Var.t * Var.t) list =
      List.filter_map
        (Var.OffsetMap.keys m1 @ Var.OffsetMap.keys m2)
        ~f:(fun k ->
          ...
          | _ ->
              ...
              None)
    in
    fvstack s1.vstack s2.vstack
    @ f s1.locals s2.locals @ f s1.globals s2.globals @ fmap s1.memory s2.memory
end

module Spec = struct
  type t = Bottom | NotBottom of SpecWithoutBottom.t
  [@@deriving compare, equal]
  ...
  let get_or_fail (s : t) : SpecWithoutBottom.t =
    match s with
    | NotBottom s -> s
  ...
  let memvars (s : t) : Var.t list =
    match s with Bottom -> [] | NotBottom s -> SpecWithoutBottom.memvars s
  ...
  let vars (data : (t * t) IntMap.t) : Var.Set.t =
    List.fold_left (IntMap.to_alist data) ~init:Var.Set.empty
  ...
  let extract_different_vars (s1 : t) (s2 : t) : (Var.t * Var.t) list =
    match (s1, s2) with
    | Bottom, _ | _, Bottom -> []
    | NotBottom s1, NotBottom s2 ->
        SpecWithoutBottom.extract_different_vars s1 s2

  let ret (i : t Instr.t) : Var.t =
    match Instr.annotation_after i with
    | Bottom -> failwith "Spec.ret bottom has no ret"
    | NotBottom s -> (
        match List.hd s.vstack with
        | Some ret -> ret
