...
module VarEq = struct
  module T = struct
    type t =
      Var.Set.t
      (* With the invariant that the size of the set is always 2 (in practice, it can be = 1 during the construction made by var_prop (e.g., if l0 = l0), but these elements will be filtred) *)
    ...
    let other_side (eq : t) (v : Var.t) : Var.t =
      ...
      else failwith "VarEq.other_side had an incorrect variable or equality"

    let contains (eq : t) (v : Var.t) : bool = Var.Set.mem eq v
  end
  ...
  module Set = struct
    ...
    let vars (vs : t) : Var.Set.t = fold vs ~init:Var.Set.empty ~f:Var.Set.union
  end
end
...
let eqs_data_instr (instr : (Instr.data, Spec.t) Instr.labelled) : VarEq.Set.t =
  ...
  | Nop | Drop | Select _ | MemorySize | MemoryGrow | MemoryCopy | MemoryFill
  | MemoryInit _ | Unary _ | Binary _ | Compare _ | Test _ | Convert _ ->
      VarEq.Set.empty
  | Const n ->
      VarEq.Set.singleton
        (VarEq.of_vars
           (Spec_inference.top (Spec.get_or_fail instr.annotation_after).vstack)
           (Var.Const n))
  | LocalGet l ->
      VarEq.Set.singleton
        (VarEq.of_vars
           (Spec_inference.top (Spec.get_or_fail instr.annotation_after).vstack)
           (Spec_inference.get l
              (Spec.get_or_fail instr.annotation_before).locals))
  | LocalSet l ->
      VarEq.Set.singleton
        (VarEq.of_vars
           (Spec_inference.get l
              (Spec.get_or_fail instr.annotation_after).locals)
           (Spec_inference.top (Spec.get_or_fail instr.annotation_before).vstack))
  | LocalTee l ->
      VarEq.Set.of_list
        [
          VarEq.of_vars
            (Spec_inference.get l
               (Spec.get_or_fail instr.annotation_after).locals)
            (Spec_inference.top
               (Spec.get_or_fail instr.annotation_before).vstack);
          VarEq.of_vars
            (Spec_inference.top (Spec.get_or_fail instr.annotation_after).vstack)
            (Spec_inference.get l
               (Spec.get_or_fail instr.annotation_before).locals);
  ...
  | GlobalGet g ->
      VarEq.Set.singleton
        (VarEq.of_vars
           (Spec_inference.top (Spec.get_or_fail instr.annotation_after).vstack)
           (Spec_inference.get g
              (Spec.get_or_fail instr.annotation_before).globals))
  | GlobalSet g ->
      VarEq.Set.singleton
        (VarEq.of_vars
           (Spec_inference.get g
              (Spec.get_or_fail instr.annotation_after).globals)
           (Spec_inference.top (Spec.get_or_fail instr.annotation_before).vstack))
  | Load _ ->
      VarEq.Set.empty
      (* TODO (it is sound to ignore them, but shouldn't be too complicated to deal with this) *)
...
let var_prop (cfg : Spec.t Cfg.t) : Spec.t Cfg.t =
  ...
  let equalities : VarEq.Set.t =
    List.fold_left (Cfg.all_blocks cfg) ~init:VarEq.Set.empty
      ~f:(fun eqs block ->
        match block.content with
        | Data instrs ->
            (* Go through the instructions of the block to gather their equalities *)
            List.fold_left instrs ~init:eqs ~f:(fun eqs instr ->
                VarEq.Set.union eqs (eqs_data_instr instr))
        | Control { instr = Merge; _ } ->
            ...
            List.fold_left (Cfg.predecessors cfg block.idx) ~init:eqs
              ~f:(fun eqs pred ->
                let pred_spec =
                  Spec.get_or_fail (Cfg.state_after_block cfg pred init_spec)
                in
                ...
                VarEq.Set.union eqs
                  (VarEq.Set.of_list
                     (List.map2_exn ~f:VarEq.of_vars pred_spec.vstack
                        spec.vstack
                     @ List.map2_exn ~f:VarEq.of_vars pred_spec.locals
                         spec.locals
                     @ List.map2_exn ~f:VarEq.of_vars pred_spec.globals
                         spec.globals)))
        | Control _ -> eqs (* No equality arises from other control blocks *))
  in
  (* Filter out tautologies *)
  let equalities =
    VarEq.Set.filter equalities ~f:(fun vs -> Var.Set.length vs = 2)
  in
  ...
  let equalities =
    VarEq.Set.filter equalities ~f:(fun vs ->
  ...
  let rec eq_classes_computation (worklist : Var.Set.t)
      (current_class : Var.Set.t) (classes : Var.Set.t list)
      (equalities : VarEq.Set.t) : Var.Set.t list =
    match Var.Set.choose worklist with
    | None -> (
        (* No more variable on the worklist, we're done with the current class *)
        match VarEq.Set.choose equalities with
        | None ->
            (* We went through all equalities, we're done *)
            List.filter (current_class :: classes) ~f:(fun c ->
                not (Var.Set.is_empty c))
        | Some eq ->
            (* Compute the next equality class *)
            eq_classes_computation eq Var.Set.empty (current_class :: classes)
              (VarEq.Set.remove equalities eq))
    | Some var ->
        if Var.Set.mem current_class var then
          (* Already in the class, we can ignore it *)
          eq_classes_computation
            (Var.Set.remove worklist var)
            current_class classes equalities
        else
          (* Add var to the current equality class,
             and add all equalities in which it is involved to the worklist *)
          let eqs =
            VarEq.Set.filter equalities ~f:(fun eq -> VarEq.contains eq var)
          in
          let vars = VarEq.Set.vars eqs in
          eq_classes_computation
            (Var.Set.union (Var.Set.remove worklist var) vars)
            (Var.Set.add current_class var)
            classes
            (VarEq.Set.diff equalities eqs)
  in
  let classes =
    eq_classes_computation Var.Set.empty Var.Set.empty [] equalities
  in
  let rewrite_target (vs : Var.Set.t) : Var.t =
    ...
    | Some v -> v
    | None -> (
        match Var.Set.min_elt vs with
        | Some v -> v
        | None -> failwith "var_prop: invalid equality class")
  in
  ...
  let fannot (annot : Spec.t) : Spec.t =
    Spec.map_vars annot ~f:(fun (v : Var.t) ->
        ...
        | None -> v)
  in
  Cfg.map_annotations cfg ~f:(fun i ->
      (fannot (Instr.annotation_before i), fannot (Instr.annotation_after i)))
...
let count_vars (cfg : Spec.t Cfg.t) : int = Var.Set.length (all_vars cfg)
...
module Test = struct
  let%test "var prop - simple test" =
    ...
    let expected =
      Var.Set.of_list
        [
          Var.Return; Var.Local 0; Var.Global 0; Var.Const (Prim_value.of_int 0);
        ]
    in
