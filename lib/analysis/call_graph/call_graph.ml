...
let find_edge (cg : t) (node : int32) (target : int32) : Edge.t option =
  ...
  | Some edges ->
      EdgeSet.find edges ~f:(fun edge -> Int32.(edge.target = target))
  | None -> None
...
let indirect_call_targets (wasm_mod : Wasm_module.t) (typ : Int32.t) :
    Int32.t list =
  ...
  | Some table ->
      ...
      let funs =
        List.map (Table_inst.indices table) ~f:(fun idx ->
            Table_inst.get table idx)
      in
      let funs_with_matching_type =
        List.filter_map funs ~f:(function
          | Some fa ->
              if Stdlib.(ftype = Wasm_module.get_func_type wasm_mod fa) then
                Some fa
              else None
          | None -> None)
      in
  ...
  | None ->
      ...
      let funs =
        List.map wasm_mod.imported_funcs ~f:(fun (idx, _, _) -> idx)
        @ List.map wasm_mod.funcs ~f:(fun f -> f.idx)
      in
      let ftype = Wasm_module.get_type wasm_mod typ in
      (* These are all the functions with a valid type *)
      List.filter funs ~f:(fun idx ->
...
let make (wasm_mod : Wasm_module.t) : t =
  ...
  let nodes =
    Int32Set.of_list
      (List.init
         (List.length wasm_mod.imported_funcs + List.length wasm_mod.funcs)
         ~f:(fun i -> Int32.of_int_exn i))
  in
  let rec collect_calls (f : Int32.t) (instr : 'a Instr.t)
      (edges : EdgeSet.t Int32Map.t) : EdgeSet.t Int32Map.t =
    match instr with
    ...
    | Control { instr = CallIndirect (_, _, _, typ); _ } ->
        List.fold_left (find_targets wasm_mod typ) ~init:edges
          ~f:(fun edges f' ->
    ...
    | _ -> edges
  and collect_calls_instrs (f : Int32.t) (instrs : 'a Instr.t list)
      (edges : EdgeSet.t Int32Map.t) : EdgeSet.t Int32Map.t =
    List.fold_left instrs ~init:edges ~f:(fun edges i ->
        collect_calls f i edges)
  in
  let edges =
    List.fold_left wasm_mod.funcs ~init:Int32Map.empty ~f:(fun edges f ->
        List.fold_left f.code.body ~init:edges ~f:(fun edges i ->
            collect_calls f.idx i edges))
  in
  { nodes; edges }
  ...
  Printf.sprintf "digraph \"Call graph\" {\n%s\n%s\n}"
    (String.concat ~sep:"\n"
       (List.map (Int32Set.to_list cg.nodes) ~f:(fun n ->
            Printf.sprintf "node%s [shape=record, label=\"{%s}\"];"
              (Int32.to_string n) (Int32.to_string n))))
    (String.concat ~sep:"\n"
       (List.concat_map (Int32Map.to_alist cg.edges) ~f:(fun (src, dsts) ->
            List.map (EdgeSet.to_list dsts) ~f:(fun dst ->
                let extra = if dst.direct then "" else "[style=dashed]" in
                Printf.sprintf "node%s -> node%s %s;\n" (Int32.to_string src)
                  (Int32.to_string dst.target)
                  extra))))
  ...
  Int32Map.iteri cg.edges ~f:(fun ~key:node ~data:edges ->
      EdgeSet.iter edges ~f:(function { target; direct } ->
          ...
          Buffer.add_string buf
            (Printf.sprintf "%s %s %s\n" (Int32.to_string node)
               (Int32.to_string target) direct_str)));
  Buffer.contents buf
          ...
          loop
            {
              ...
              edges =
                Int32Map.add_exn new_cg.edges ~key:node
                  ~data:(EdgeSet.union old_edges edges_to_add);
            }
  in
...
let remove_imports (cg : t) (nimports : Int32.t) : t =
  ...
  let edges =
    Int32Map.filter_mapi cg.edges ~f:(fun ~key:src ~data:dsts ->
        if Int32.(src >= nimports) then
          let dsts' =
            EdgeSet.filter dsts ~f:(fun edge -> Int32.(edge.target >= nimports))
          in
          if EdgeSet.is_empty dsts' then None else Some dsts'
        else
    ...
    EdgeSet.iter
      ...
      ~f:(fun w ->
        ...
        | None ->
            ...
            lowlinks :=
              Int32Map.set !lowlinks ~key:v
                ~data:
                  (Int32.min
                     (Int32Map.find_exn !lowlinks v)
                     (Int32Map.find_exn !lowlinks w.target))
        | Some windex -> (
            match Int32Map.find !on_stack w.target with
            | Some true ->
                lowlinks :=
                  Int32Map.set !lowlinks ~key:v
    ...
    if Int32.(Int32Map.find_exn !lowlinks v = Int32Map.find_exn !indices v) then
      let rec make_scc (scc : Int32.t list) : Int32.t list =
        ...
        | w :: rest ->
            ...
            if Int32.(w = v) then w :: scc else make_scc (w :: scc)
      in
      sccs := make_scc [] :: !sccs
  in
  Int32Set.iter cg.nodes ~f:(fun v ->
      match Int32Map.find !indices v with
      | Some _ -> ()
