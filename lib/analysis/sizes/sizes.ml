...
let patch (s : stream) (pos : int) (b : char) : unit =
  s.patches := (pos, b) :: !(s.patches)
...
let buf_size (s : stream) : int = Bytes.length (buf_to_bytes s)
...
let generate_binary (m : Wasm_module.t) (filename : string option) : t =
  ...
  let module E = struct
    ...
    let table_type (t : Table.table_type) =
      elem_type;
      limits vu32 t
    ...
    let global_type (t : Global.global_type) =
      value_type t.typ;
    ...
    let rec instr (e : unit Instr.t) =
      ...
      | Data i -> (
          ...
          | GlobalSet x ->
              op 0x24;
              var x
          | Load ({ typ = I32; pack = None; _ } as mo) ->
              op 0x28;
              memop mo
          | Load ({ typ = I64; pack = None; _ } as mo) ->
              op 0x29;
              memop mo
          | Load ({ typ = F32; pack = None; _ } as mo) ->
              op 0x2a;
              memop mo
          | Load ({ typ = F64; pack = None; _ } as mo) ->
              op 0x2b;
              memop mo
          | Load ({ typ = I32; pack = Some (Pack8, SX); _ } as mo) ->
              op 0x2c;
              memop mo
          | Load ({ typ = I32; pack = Some (Pack8, ZX); _ } as mo) ->
              op 0x2d;
              memop mo
          | Load ({ typ = I32; pack = Some (Pack16, SX); _ } as mo) ->
              op 0x2e;
              memop mo
          | Load ({ typ = I32; pack = Some (Pack16, ZX); _ } as mo) ->
              op 0x2f;
              memop mo
          | Load { typ = I32; pack = Some (Pack32, _); _ } -> assert false
          | Load ({ typ = I64; pack = Some (Pack8, SX); _ } as mo) ->
              op 0x30;
              memop mo
          | Load ({ typ = I64; pack = Some (Pack8, ZX); _ } as mo) ->
              op 0x31;
              memop mo
          | Load ({ typ = I64; pack = Some (Pack16, SX); _ } as mo) ->
              op 0x32;
              memop mo
          | Load ({ typ = I64; pack = Some (Pack16, ZX); _ } as mo) ->
              op 0x33;
              memop mo
          | Load ({ typ = I64; pack = Some (Pack32, SX); _ } as mo) ->
              op 0x34;
              memop mo
          | Load ({ typ = I64; pack = Some (Pack32, ZX); _ } as mo) ->
              op 0x35;
              memop mo
          | Load { typ = F32 | F64; pack = Some _; _ } -> assert false
          | Store ({ typ = I32; pack = None; _ } as mo) ->
              op 0x36;
              memop mo
          | Store ({ typ = I64; pack = None; _ } as mo) ->
              op 0x37;
              memop mo
          | Store ({ typ = F32; pack = None; _ } as mo) ->
              op 0x38;
              memop mo
          | Store ({ typ = F64; pack = None; _ } as mo) ->
              op 0x39;
              memop mo
          | Store ({ typ = I32; pack = Some (Pack8, _); _ } as mo) ->
              op 0x3a;
              memop mo
          | Store ({ typ = I32; pack = Some (Pack16, _); _ } as mo) ->
              op 0x3b;
              memop mo
          | Store { typ = I32; pack = Some (Pack32, _); _ } -> assert false
          | Store ({ typ = I64; pack = Some (Pack8, _); _ } as mo) ->
              op 0x3c;
              memop mo
          | Store ({ typ = I64; pack = Some (Pack16, _); _ } as mo) ->
              op 0x3d;
              memop mo
          | Store ({ typ = I64; pack = Some (Pack32, _); _ } as mo) ->
              op 0x3e;
              memop mo
          ...
          | Convert { typ = I32; op = Convertop.TruncSatSF32 } ->
              op 0xfc;
              op 0x00
          | Convert { typ = I32; op = Convertop.TruncSatUF32 } ->
              op 0xfc;
              op 0x01
          | Convert { typ = I32; op = Convertop.TruncSatSF64 } ->
              op 0xfc;
              op 0x02
          | Convert { typ = I32; op = Convertop.TruncSatUF64 } ->
              op 0xfc;
              op 0x03
          ...
          | Convert { typ = I64; op = Convertop.TruncSatSF32 } ->
              op 0xfc;
              op 0x04
          | Convert { typ = I64; op = Convertop.TruncSatUF32 } ->
              op 0xfc;
              op 0x05
          | Convert { typ = I64; op = Convertop.TruncSatSF64 } ->
              op 0xfc;
              op 0x06
          | Convert { typ = I64; op = Convertop.TruncSatUF64 } ->
              op 0xfc;
              op 0x07
    ...
    let import (im : Import.t) =
      name im.module_name;
      name im.item_name;
      import_desc im.idesc

    let import_section ims = section 2 (vec import) ims (ims <> [])

    (* Function section *)
    let func (f : Func_inst.t) = var f.type_idx
    let func_section (fs : Func_inst.t list) = section 3 (vec func) fs (fs <> [])

    (* Table section *)
    let table (tab : Table.t) = table_type tab.ttype
    let table_section tabs = section 4 (vec table) tabs (tabs <> [])

    (* Memory section *)
    let memory (mem : Memory.t) = memory_type mem.mtype
    ...
    let export (ex : Export.t) =
      name ex.name;
      export_desc ex.edesc

    let export_section exs = section 7 (vec export) exs (exs <> [])

    (* Start section *)
    let start_section xo = section 8 (opt var) xo (xo <> None)
    ...
    (* Element section *)
    let elem (_ : Elem_segment.t) = failwith "NYI"
    ...
    let data (seg : Data_segment.t) =
      ...
      | Segment_mode.Active { table_index; offset } ->
          ...
          const offset;
          string seg.dinit
      | Segment_mode.Declarative -> failwith "illegal declarative data segment"
  ...
  (match filename with Some f -> buf_to_file s f | None -> ());
  sizes
