...
module type INTRA = sig
  ...
  val analyze_keep :
    ...
    annot_expected Cfg.t ->
    summary Int32Map.t ->
    (annot_expected * state) Cfg.t * summary
...
module Make (Transfer : Transfer.TRANSFER) (* : INTRA *) = struct
  ...
  type result =
    ...
    | Branch of state * state
        (** Upon a `brif`, there are two successor states: one where the condition holds, and where where it does not hold. This is used to model that. *)
  [@@deriving compare]
  ...
  let result_to_state (cfg : annot_expected Cfg.t) (r : result) : state =
    match r with
  ...
  let analyze_ (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (summaries : summary Int32Map.t) : intra_results =
    ...
    let block_data : (result * result) IntMap.t ref =
      ref
        (IntMap.of_alist_exn
           (List.map
              (IntSet.to_list (Cfg.all_block_indices cfg))
              ~f:(fun idx -> (idx, (bottom, bottom)))))
    in
    ...
    let instr_data : intra_results ref =
      ref
        (Instr.Label.Map.of_alist_exn
           (List.map
              (Instr.Label.Set.to_list (Cfg.all_instruction_labels cfg))
              ~f:(fun idx -> (idx, (bottom, bottom)))))
    in
    ...
    let transfer (b : 'a Basic_block.t) (state : Transfer.state) : result =
      ...
      | Data instrs ->
          Simple
            (List.fold_left instrs ~init:state ~f:(fun prestate instr ->
                 let poststate =
                   Transfer.data_instr_transfer module_ cfg instr prestate
                 in
                 instr_data :=
                   Instr.Label.Map.set !instr_data ~key:instr.label
                     ~data:(Simple prestate, Simple poststate);
                 poststate))
      | Control instr ->
          let poststate =
            match
              Transfer.control_instr_transfer module_ summaries cfg instr state
            with
            | `Simple s -> Simple s
    ...
    let analyze_block (block_idx : int) : Transfer.state * result =
      ...
      let pred_states =
        List.map incoming ~f:(fun (idx, d) ->
            match (snd (IntMap.find_exn !block_data idx), d) with
            | Simple s, _ -> (idx, s)
            ...
            | Branch _, None ->
                failwith
                  (Printf.sprintf
                     "invalid branch state at block %d, from block %d" block_idx
                     idx)
            | Uninitialized, _ -> (idx, Transfer.bottom_state cfg))
      in
    ...
    let join_result (r1 : result) (r2 : result) =
      ...
      | Branch (st1, st2), Branch (st1', st2') ->
          Branch (Transfer.join_state st1 st1', Transfer.join_state st2 st2')
    ...
    let widen_result (r1 : result) (r2 : result) =
      ...
      | Uninitialized, _ -> r2
      | _, Uninitialized -> r1
      | Simple st1, Simple st2 -> Simple (Transfer.widen_state st1 st2)
      | Branch (st1, st2), Branch (st1', st2') ->
          Branch (Transfer.widen_state st1 st1', Transfer.widen_state st2 st2')
        ...
        | _ ->
            ...
            let new_out_state =
              ...
              if IntSet.mem cfg.loop_heads block_idx then
                widen_result previous_out_state
                  (join_result previous_out_state out_state)
              else join_result previous_out_state out_state
            in
            block_data :=
              IntMap.set !block_data ~key:block_idx
                ~data:(Simple in_state, new_out_state);
            (* And recurse by adding all successors *)
            let successors = Cfg.successors cfg block_idx in
            fixpoint
              (IntSet.union
                 (IntSet.remove worklist block_idx)
                 (IntSet.of_list successors))
              (iteration + 1)
    in
    (* Performs narrowing by re-analyzing once each block *)
    let rec _narrow (blocks : int list) : unit =
      match blocks with
      | [] -> ()
      | block_idx :: blocks ->
          let in_state, out_state = analyze_block block_idx in
          block_data :=
            IntMap.set !block_data ~key:block_idx
              ~data:(Simple in_state, out_state);
          _narrow blocks
    in
  ...
  let analyze (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (summaries : summary Int32Map.t) : state Cfg.t * summary =
    let to_state (results_pair : result * result) : state * state =
      ( result_to_state cfg (fst results_pair),
        result_to_state cfg (snd results_pair) )
    in
    let instr_data = analyze_ module_ cfg summaries in
    let analyzed_cfg =
      Cfg.map_annotations cfg ~f:(fun i ->
          to_state (Instr.Label.Map.find_exn instr_data (Instr.label i)))
    in
    let summary = Transfer.extract_summary cfg analyzed_cfg in
    (analyzed_cfg, summary)

  let analyze_keep (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (summaries : summary Int32Map.t) :
      (annot_expected * state) Cfg.t * summary =
    let to_state_keep (previous : annot_expected * annot_expected)
        (results_pair : result * result) :
        (annot_expected * state) * (annot_expected * state) =
      ( (fst previous, result_to_state cfg (fst results_pair)),
        (snd previous, result_to_state cfg (snd results_pair)) )
    in
    let to_state (results_pair : result * result) : state * state =
      ( result_to_state cfg (fst results_pair),
        result_to_state cfg (snd results_pair) )
    ...
    let analyzed_cfg =
      Cfg.map_annotations cfg ~f:(fun i ->
          to_state (Instr.Label.Map.find_exn instr_data (Instr.label i)))
    in
    let analyzed_keep_cfg =
      Cfg.map_annotations cfg ~f:(fun i ->
          to_state_keep
            (Instr.annotation_before i, Instr.annotation_after i)
            (Instr.Label.Map.find_exn instr_data (Instr.label i)))
    in
    let summary = Transfer.extract_summary cfg analyzed_cfg in
    (analyzed_keep_cfg, summary)

  (** Extract the out state from intra-procedural results *)
  let final_state (cfg_before : annot_expected Cfg.t) (cfg : state Cfg.t) :
      state =
