...
module Taint = struct
  ...
  type t = Taints of Var.Set.t | TopTaint [@@deriving sexp, compare, equal]
  ...
  let join (t1 : t) (t2 : t) : t =
    match (t1, t2) with
    | TopTaint, _ | _, TopTaint -> TopTaint
  ...
  let to_string (t : t) : string =
    match t with
    | Taints t ->
        if Var.Set.is_empty t then "_"
        else
          String.concat ~sep:"," (List.map (Var.Set.to_list t) ~f:Var.to_string)
    | TopTaint -> "TopTaint"
  ...
  let of_string (s : string) : t =
    ...
    | _ ->
        Taints
          (Var.Set.of_list
             (List.map (String.split s ~on:',') ~f:(fun s ->
  ...
  let substitute (t : t) (subst : (Var.t * t) list) : t =
    match t with
    | Taints ts ->
        ...
        let vars_to_remove, taint_to_add =
          Var.Set.fold ts ~init:(Var.Set.empty, bottom) ~f:(fun (rm, add) x ->
              match List.find subst ~f:(fun (y, _) -> Var.equal x y) with
              | None -> (rm, add)
              | Some (_, z) -> (Var.Set.add rm x, join z add))
        in
  ...
  module Test = struct
    ...
    let%test "taint joining works as expected" =
      ...
      && equal
           (join (taint (Var.Local 0)) (taint (Var.Local 1)))
           (taints (Var.Set.of_list [ Var.Local 0; Var.Local 1 ]))
    ...
    let%test "taint to_string works as expected" =
      ...
      && String.equal "l0,l1"
           (to_string (taints (Var.Set.of_list [ Var.Local 0; Var.Local 1 ])))
...
type t = Taint.t Var.Map.t [@@deriving sexp, compare, equal]
...
let top (globals : Var.t list) (ret : Var.t option) : t =
  ...
  in
  Option.fold ret
    ~init:(List.fold_left globals ~init:bottom ~f:set_top_taint)
    ~f:set_top_taint
...
let subsumes (t1 : t) (t2 : t) : bool =
  ...
  Var.Map.fold t2 ~init:true ~f:(fun ~key:k ~data:v2 sub ->
      if sub then
        if Taint.equal v2 Taint.bottom then true
          (* v2 is bottom, so any value in t1[k] would subsume it, no need to check *)
        else
...
let full_to_string (s : t) : string =
  Printf.sprintf "[%s]"
    (String.concat ~sep:", "
       (List.map (Var.Map.to_alist s) ~f:(fun (k, t) ->
            Printf.sprintf "%s: %s" (Var.to_string k) (Taint.to_string t))))
...
let only_non_id_to_string (s : t) : string =
  let restricted =
    Var.Map.filteri s ~f:(fun ~key:k ~data:d ->
        match d with
...
let join (s1 : t) (s2 : t) : t =
  Var.Map.merge s1 s2 ~f:(fun ~key:_ v ->
      match v with
...
let get_taint (s : t) (var : Var.t) : Taint.t =
  match Var.Map.find s var with Some t -> t | None -> Taints Var.Set.empty
...
module Test = struct
  let%test "subsumption works as expected" =
    ...
    let bot =
      Var.Map.of_alist_exn
        [
          (Var.Local 0, Taint.taint (Var.Local 0));
          (Var.Local 1, Taint.taint (Var.Local 1));
          (Var.Var (lab 0), Taint.bottom);
        ]
    in
    (* [i0 -> l0] *)
    let l0 =
      Var.Map.of_alist_exn
        [
          (Var.Local 0, Taint.taint (Var.Local 0));
          (Var.Local 1, Taint.taint (Var.Local 1));
          (Var.Var (lab 0), Taint.taint (Var.Local 0));
        ]
    in
    (* [i0 -> l1] *)
    let l1 =
      Var.Map.of_alist_exn
        [
          (Var.Local 0, Taint.taint (Var.Local 0));
          (Var.Local 1, Taint.taint (Var.Local 1));
          (Var.Var (lab 0), Taint.taint (Var.Local 1));
        ]
    in
    (* [i0 -> top] *)
    let top =
    ...
    let l0l1 =
      Var.Map.of_alist_exn
        [
          ...
          (Var.Local 1, Taint.taint (Var.Local 1));
          ( Var.Var (lab 0),
            Taint.taints (Var.Set.of_list [ Var.Local 0; Var.Local 1 ]) );
        ]
    in
  ...
  let%test "join produces the expected taint maps" =
    ...
    in
    equal (join bot l0) l0
    && equal (join l0 bot) l0
    && equal (join l0 l1) l0l1
    && equal (join l1 l0) l0l1
    && equal (join l0 l0l1) l0l1
    && equal (join l0l1 l0) l0l1
    && equal (join bot top) top
    && equal (join top bot) top
    && equal (join top l0) top
    && equal (join l0 top) top
