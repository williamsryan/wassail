...
module Make (* : Transfer.TRANSFER *) = struct
  ...
  type state = Taint_domain.t [@@deriving sexp, compare, equal]
  (** The state *)
  ...
  *)
  let taint_specifications :
      (Taint_domain.Taint.t * Taint_domain.Taint.t list) StringMap.t ref =
    let bottom = Taint_domain.Taint.Taints Var.Set.empty in
    ref
      (StringMap.of_alist_exn
         [
           ( "fgets",
             ( Taint_domain.Taint.Taints (Var.Set.singleton (Var.Other "fgets")),
               [
                 Taint_domain.Taint.Taints
                   (Var.Set.singleton (Var.Other "fgets"));
                 bottom;
                 bottom;
               ] ) );
         ])
  ...
  let init_state (cfg : 'a Cfg.t) : state =
    Var.Map.of_alist_exn
      (List.mapi cfg.arg_types ~f:(fun i _ ->
           (Var.Local i, Taint_domain.Taint.taint (Var.Local i)))
      @ List.mapi cfg.global_types ~f:(fun i _ ->
            (Var.Global i, Taint_domain.Taint.taint (Var.Global i))))
  ...
  let data_instr_transfer (_module_ : Wasm_module.t)
      (_cfg : annot_expected Cfg.t) (i : annot_expected Instr.labelled_data)
      (state : state) : state =
    let ret (i : annot_expected Instr.labelled_data) : Var.t =
      match List.hd (Spec.get_or_fail i.annotation_after).vstack with
      | Some r -> r
      | None -> failwith "Taint: no return value"
    in
    match i.instr with
    | Nop | MemorySize | Drop | MemoryGrow -> state
    | MemoryCopy | MemoryFill | MemoryInit _ ->
        state (* Not model entirely properly *)
    ...
    | LocalGet l ->
        Taint_domain.add_taint_v state (ret i)
          (get_nth (Spec.get_or_fail i.annotation_before).locals l)
    | LocalSet l ->
        Taint_domain.add_taint_v state
          (get_nth (Spec.get_or_fail i.annotation_before).locals l)
          (pop (Spec.get_or_fail i.annotation_before).vstack)
    | LocalTee l ->
    ...
    | GlobalGet g ->
        Taint_domain.add_taint_v state (ret i)
          (get_nth (Spec.get_or_fail i.annotation_before).globals g)
    | GlobalSet g ->
        Taint_domain.add_taint_v state
          (get_nth (Spec.get_or_fail i.annotation_before).globals g)
          (pop (Spec.get_or_fail i.annotation_before).vstack)
    ...
    | Load { offset = _offset; _ } ->
        ...
        let locs =
          (* if !Taint_options.use_relational then
        ...
        *)
        let taints =
          List.map locs ~f:(fun (k, offset) ->
              (* Log.warn
                 (Printf.sprintf "TODO: currently ignoring offsets in taints!!!\n--------------------\n--------------\n"); (* maybe only the values should be tainted, not the keys *) *)
              Taint_domain.Taint.join
                (Taint_domain.get_taint state k)
                (Taint_domain.get_taint state
                   (Var.OffsetMap.find_exn mem (k, offset))))
        in
        Taint_domain.add_taint state (ret i)
          (* ret is the join of all these taints *)
          (List.fold_left taints ~init:Taint_domain.Taint.bottom
             ~f:Taint_domain.Taint.join)
    | Store { offset = _offset; _ } ->
        ...
        List.fold_left locs ~init:state ~f:(fun s (k, offset) ->
            ...
            Taint_domain.add_taint_v
              (Taint_domain.add_taint_v s k vval)
              (Var.OffsetMap.find_exn mem (k, offset))
              vval)
  ...
  let control_instr_transfer
      ...
      (state : state) : [ `Simple of state | `Branch of state * state ] =
    (* The pre state *)
    let apply_summary (f : Int32.t) (arity : int * int) (state : state) : state
      ...
      | None ->
          if Int32.(f < module_.nfuncimports) then (
            Log.warn
              (Printf.sprintf
                 "No summary found for function %ld (imported function): \
                  assuming taint is preserved"
                 f);
            state)
          else
      ...
      | Some summary -> (
          let args =
            List.take (Spec.get_or_fail i.annotation_before).vstack (fst arity)
          in
          let ret =
            if snd arity = 1 then
              List.hd (Spec.get_or_fail i.annotation_after).vstack
            else None
          in
          let taint_after_call =
            Taint_summary.apply summary state args
              (Spec.get_or_fail i.annotation_before).globals
              (Spec.get_or_fail i.annotation_after).globals
              (List.concat_map
                 (Var.OffsetMap.to_alist
                    (Spec.get_or_fail i.annotation_after).memory)
                 ~f:(fun ((a, _offset), b) ->
          ...
          in
          let export =
            List.find module_.exported_funcs ~f:(fun (id, _, _) ->
                Int32.(id = f))
          in
          match export with
          | Some (_, fname, _) -> (
              Log.info (Printf.sprintf "function is named %s" fname);
              match (ret, StringMap.find !taint_specifications fname) with
              | Some ret_var, Some (ret_taint, args_taint) ->
                  ...
                  List.fold_left
                    (List.zip_exn (List.rev args) args_taint)
                    ~init:
                      (Taint_domain.add_taint taint_after_call ret_var ret_taint)
                    ~f:(fun state (var, taint) ->
                      Printf.printf "adding taint %s to var %s\n"
                        (Taint_domain.Taint.to_string taint)
                        (Var.to_string var);
                      Taint_domain.add_taint state var taint)
              | _ -> taint_after_call)
          | None -> taint_after_call)
    in
    match i.instr with
    ...
    | CallIndirect (_, arity, _, typ) ->
        ...
        (* Apply the summaries *)
        `Simple
          (List.fold_left targets ~init:state ~f:(fun acc idx ->
               Taint_domain.join (apply_summary idx arity state) acc))
    | Br _ -> `Simple state
  ...
  let merge_flows (_module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
      (block : annot_expected Basic_block.t) (states : (int * state) list) :
      state =
    let init_spec =
      Spec_inference.init_state cfg
      (* , Relational_transfer.bottom_state (Cfg.map_annotations cfg ~f:(fun i -> fst (Instr.annotation_before i), fst (Instr.annotation_after i)))*)
    in
    match states with
    | [] -> init_state cfg
    | _ -> (
        ...
        | Control { instr = Merge; _ } ->
            ...
            let states' =
              List.map states ~f:(fun (idx, s) ->
                  ...
                  List.fold_left
                    (Spec_inference.extract_different_vars spec spec') ~init:s
                    ~f:(fun s (x, y) ->
                      (* TODO: should it be x y or y x? *)
                      Taint_domain.add_taint_v s x y))
            in
  ...
  let summary (cfg : annot_expected Cfg.t) (out_state : state) : summary =
    ...
    | Bottom ->
        (* The function exit is likely unreachable, so we use a bottom summary *)
        {
          ret = None;
          globals =
            List.init (List.length cfg.global_types) ~f:(fun _ ->
                Taint_domain.Taint.bottom);
          mem = Taint_domain.Taint.bottom;
        }
    | NotBottom exit_spec ->
        Taint_summary.make cfg out_state
          (if List.length cfg.return_types = 1 then List.hd exit_spec.vstack
           else None)
          exit_spec.globals
  ...
  let extract_summary (cfg : annot_expected Cfg.t) (analyzed_cfg : state Cfg.t)
      : summary =
    let out_state =
      Cfg.state_after_block analyzed_cfg cfg.exit_block (init_state cfg)
    in
