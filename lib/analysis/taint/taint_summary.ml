...
type t = {
  ret : Taint_domain.Taint.t option;
      (** The taint of the (optional) return value *)
  globals : Taint_domain.Taint.t list;
      (** The taint of the globals after applying the function *)
...
let subsumes (s1 : t) (s2 : t) : bool =
  if List.length s1.globals = List.length s2.globals then
    ...
    let globals_subsumes =
      ret_subsumes
      && List.fold2_exn s1.globals s2.globals ~init:true ~f:(fun acc t1 t2 ->
             acc && Taint_domain.Taint.subsumes t1 t2)
    in
    let mem_subsumes =
      globals_subsumes && Taint_domain.Taint.subsumes s1.mem s2.mem
    in
    mem_subsumes
  else false
...
let to_string (s : t) : string =
  Printf.sprintf "stack: [%s], globals: [%s], mem: %s"
    (String.concat ~sep:";"
       (List.map ~f:Taint_domain.Taint.to_string (Option.to_list s.ret)))
    (String.concat ~sep:";"
       (List.map ~f:Taint_domain.Taint.to_string s.globals))
    (Taint_domain.Taint.to_string s.mem)

let of_string (s : string) : t =
  let stack, globals, mem =
    Scanf.sscanf s "stack: [%s], globals: [%s], mem: %s" (fun x y z ->
        (x, y, z))
  in
  let ret =
    match String.split stack ~on:';' with
    | [] -> None
    | taint :: [] -> Some (Taint_domain.Taint.of_string taint)
    | _ -> failwith "invalid taint summary"
  in
  let globals =
    List.map (String.split globals ~on:';') ~f:Taint_domain.Taint.of_string
  in
...
let bottom (cfg : 'a Cfg.t) (_vars : Var.Set.t) : t =
  let globals =
    List.map cfg.global_types ~f:(fun _ -> Taint_domain.Taint.bottom)
  in
  let ret =
...
let top (cfg : 'a Cfg.t) (_vars : Var.Set.t) : t =
  let globals =
    List.map cfg.global_types ~f:(fun _ -> Taint_domain.Taint.top)
  in
  let ret =
    match cfg.return_types with
    | [] -> None
...
let of_import (name : string) (nglobals : Int32.t) (_args : Type.t list)
    (ret : Type.t list) : t =
  match name with
  | "fd_write" | "fd_close" | "fd_seek" | "fd_fdstat_get" | "proc_exit" ->
      (* Globals are unchanged *)
      let globals =
        List.init (Int32.to_int_exn nglobals) ~f:(fun i ->
      ...
      (* XXX: We assume memory does not get tainted, which is definitely not always true (e.g., fd_write) *)
  | _ ->
...
let initial_summaries (cfgs : 'a Cfg.t Int32Map.t) (module_ : Wasm_module.t)
    (typ : [ `Bottom | `Top ]) : t Int32Map.t =
  List.fold_left module_.imported_funcs
    ~init:
      (Int32Map.map cfgs ~f:(fun cfg ->
           (match typ with `Bottom -> bottom | `Top -> top) cfg Var.Set.empty))
    ~f:(fun summaries (idx, name, (args, ret)) ->
      Int32Map.set summaries ~key:idx
        ~data:(of_import name module_.nglobals args ret))

let make (_cfg : 'a Cfg.t) (state : Taint_domain.t) (ret : Var.t option)
    (globals_post : Var.t list) (mem_post : Var.t list) : t =
  let globals =
    List.map globals_post ~f:(fun g -> Taint_domain.get_taint state g)
  in
  let ret = Option.map ret ~f:(fun r -> Taint_domain.get_taint state r) in
  {
    ...
    mem =
      List.fold_left
        (List.map mem_post ~f:(Taint_domain.get_taint state))
 ...
 *)
let apply (summary : t) (state : Taint_domain.t) (args : Var.t list)
    (globals_pre : Var.t list) (globals_post : Var.t list)
    (mem_post : Var.t list) (ret : Var.t option) : Taint_domain.t =
  ...
  let summary_globals =
    List.init (List.length globals_pre) ~f:(fun i -> Var.Global i)
  in
  let subst (t : Taint_domain.Taint.t) : Taint_domain.Taint.t =
    ...
    Taint_domain.Taint.substitute t
      (List.map2_exn summary_args args ~f:(fun x y ->
           (x, Taint_domain.get_taint state y))
      @ List.map2_exn summary_globals globals_pre ~f:(fun x y ->
            (x, Taint_domain.get_taint state y)))
  in
  (* First, propagate the taint for the return value in the given state *)
  let with_ret =
    match (summary.ret, ret) with
    | Some r, Some r' ->
  ...
  let with_globals =
    List.fold_left
      (List.map2_exn summary.globals globals_post ~f:(fun x y -> (x, y)))
      ~init:with_ret
      ~f:(fun acc (g, g') ->
  ...
  let with_mem =
    if !Taint_options.keep_memory then
