...
let analyze_intra :
    Wasm_module.t ->
    Int32.t list ->
    (Summary.t * Domain.t Cfg.t option) Int32Map.t =
  Analysis_helpers.mk_intra
    (fun cfgs wasm_mod ->
      Int32Map.map
        ~f:(fun x -> (x, None))
        (Summary.initial_summaries cfgs wasm_mod `Bottom))
...
let annotate (wasm_mod : Wasm_module.t) (summaries : Summary.t Int32Map.t)
...
let check (expected : Summary.t) (actual : Summary.t) : bool =
  if Summary.subsumes actual expected then
    if Summary.equal actual expected then true
    else (
      Printf.printf
        "\n[IMPRECISION] summaries not equal:\nexpected: %s\nactual: %s\n"
        (Summary.to_string expected)
        (Summary.to_string actual);
...
let analyze_inter :
    Wasm_module.t ->
    Int32.t list list ->
    (Spec.t Cfg.t * Taint_domain.t Cfg.t * Summary.t) Int32Map.t =
...
let find_sinks_from_names (module_ : Wasm_module.t) (names : StringSet.t) :
    Int32Set.t =
  let funs =
    List.filter_map module_.funcs ~f:(fun f ->
      ...
      Summary.
        {
          ...
          globals = [ Domain.Taint.taint (Var.Global 0) ];
        }
    in
      ...
      Summary.
        {
          ...
          mem = Domain.Taint.taint (Var.Local 0);
          globals = [ Domain.Taint.taint (Var.Global 0) ];
        }
    in
    check expected actual
