...
type state = Taintcall_domain.t [@@deriving sexp, compare, equal]

let bottom : state = Taintcall_domain.bottom

let init_state (cfg : 'a Cfg.t) : state =
  (Taintcall_domain.Call.bottom, TaintTransfer.init_state cfg)

let bottom_state (cfg : 'a Cfg.t) : state =
  (Taintcall_domain.Call.bottom, TaintTransfer.bottom_state cfg)
...
let data_instr_transfer (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
    (i : annot_expected Instr.labelled_data) (state : state) : state =
...
let control_instr_transfer
  ...
  (* The pre state *)
  let apply_summary (f : Int32.t) (argsv : Taint_domain.Taint.t list) :
      Taintcall_domain.Call.t =
        ...
        let substituted_summary =
          Int32Map.map (fst summary) ~f:(fun args ->
              List.map args ~f:(fun arg ->
                  Taint_domain.Taint.substitute arg
                    (List.mapi argsv ~f:(fun i argv -> (Var.Local i, argv)))))
        in
        Taintcall_domain.Call.join substituted_summary (fst state)
  in
  let add_call (to_state : Taintcall_domain.Call.t) (f : Int32.t)
      (arity : int * int) : Taintcall_domain.Call.t =
    let args =
  ...
  in
  match
    TaintTransfer.control_instr_transfer module_
      (Int32Map.map ~f:snd summaries)
      cfg i (snd state)
  with
  | `Simple sndstate' -> (
      let apply_fun f arity =
        ...
        ( add_call
            (apply_summary f
               (List.map args ~f:(Taint_domain.get_taint (snd state))))
            f arity,
          sndstate' )
      in
      match i.instr with
      ...
      | CallIndirect (_, arity, _, typ) ->
          let funs_with_matching_type =
            Call_graph.indirect_call_targets module_ typ
          in
          `Simple
            (List.fold_left funs_with_matching_type ~init:state
               ~f:(fun acc idx ->
...
let extract_summary (cfg : annot_expected Cfg.t) (analyzed_cfg : state Cfg.t) :
    summary =
  let out_state =
    Cfg.state_after_block analyzed_cfg analyzed_cfg.exit_block (init_state cfg)
  in
  (fst out_state, TaintTransfer.summary cfg (snd out_state))

let merge_flows (module_ : Wasm_module.t) (cfg : annot_expected Cfg.t)
    (block : annot_expected Basic_block.t) (states : (int * state) list) : state
    =
  match
    List.reduce
      (List.map ~f:(fun x -> fst (snd x)) states)
      ~f:Taintcall_domain.Call.join
  with
  | None -> init_state cfg
  | Some s' ->
      ( s',
        TaintTransfer.merge_flows module_ cfg block
          (List.map states ~f:(fun x -> (fst x, snd (snd x)))) )
