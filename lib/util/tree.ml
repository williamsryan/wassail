...
module T = struct
  type t = {
    children : IntSet.t IntMap.t;
        (* Representation of the tree in a forward manner: edges from a node to children *)
    parent : int IntMap.t;
        (* Representation of the tree in a backward manner: edge from a node to its parent *)
...
let children (tree : t) (node : int) : IntSet.t =
  ...
  | None ->
      failwith
        (Printf.sprintf "No children found in tree %s, node %d" (to_string tree)
           node)
...
let revert_tree_representation (entry : int) (tree : IntSet.t IntMap.t) :
    int IntMap.t =
  let rec visit (rtree : int IntMap.t) (node : int) =
    let children =
      match IntMap.find tree node with
      | Some c -> c
      | None ->
          failwith
            (Printf.sprintf
               "no children found for node %d when reverting tree computation"
               node)
    in
    let rtree_with_children =
      IntSet.fold children ~init:rtree ~f:(fun rtree child ->
          IntMap.add_exn rtree ~key:child ~data:node)
    in
...
let of_children_map (entry : int) (children_map : (int * int list) list) : t =
  ...
  if check_validity_of_children_map then (
    ...
    let all_children =
      List.fold_left children_map ~init:IntSet.empty ~f:(fun acc (_, cs) ->
          IntSet.union acc (IntSet.of_list cs))
    in
    if not (IntSet.for_all all_children ~f:(fun c -> IntSet.mem all_keys c))
    then
      failwith
        (Printf.sprintf
           "of_children_map: tree spec missing nodes"
           (* "  where not all nodes are specified: %s" (String.concat ~sep:" | " (List.map children_map ~f:(fun (p, cs) -> Printf.sprintf "%d -> %s" p (String.concat ~sep:"," (List.map cs ~f:string_of_int))))) *));
    ...
    let all_children_l =
      List.fold_left children_map ~init:[] ~f:(fun acc (_, cs) -> cs @ acc)
    in
    if not (IntSet.length all_children = List.length all_children_l) then
      ...
      failwith
        (Printf.sprintf
           "of_children_map: invalid tree spec where some children have \
            multiple parents: %s"
           (String.concat ~sep:" | "
              (List.map children_map ~f:(fun (p, cs) ->
                   Printf.sprintf "%d -> %s" p
                     (String.concat ~sep:"," (List.map cs ~f:string_of_int)))))));
  let children =
    IntMap.map (IntMap.of_alist_exn children_map) ~f:IntSet.of_list
  in
  {
    children;
...
let of_node (node : int) : t = of_children_map node [ (node, []) ]

(** Extract the parent of a node in constant time *)
let parent (tree : t) (node : int) : int option =
  if node = tree.entry then None else IntMap.find tree.parent node

(** Just like parent, but throws an exception if there is no parent *)
let parent_exn (tree : t) (node : int) : int =
...
let set_root_parent (new_parent : int) (tree : t) : t =
  {
    ...
    children =
      IntMap.add_exn tree.children ~key:new_parent
        ~data:(IntSet.singleton tree.entry);
  }

(** Changes the parent of a node *)
let set_parent (tree : t) (node : int) (new_parent : int) : t =
  let disconnected_from_old_parent =
    match parent tree node with
    | Some old_parent ->
        IntMap.update tree.children old_parent ~f:(function
  ...
  {
    tree with
    parent = IntMap.update tree.parent node ~f:(fun _ -> new_parent);
    children =
      IntMap.update disconnected_from_old_parent new_parent ~f:(function
...
let nodes_between (tree : t) (x : int) (y : int) : int list option =
  let rec loop (node : int) (acc : int list) : int list option =
    match parent tree node with
    | None -> None
...
let nca (tree : t) (node1 : int) (node2 : int) : int option =
  if node1 = node2 then Some node2
  else
    (* Compute ancestors of node1 (including node1) *)
    let n1_ancestors =
      IntSet.add (fold_ancestors tree node1 IntSet.empty IntSet.add) node1
    in
...
let merge (ts : t list) (parent : int) : t =
  match ts with
  | [] -> of_node parent
  ...
  | _ :: _ ->
      ...
      let temp_tree =
        List.reduce_exn ts ~f:(fun t1 t2 ->
            {
              children =
                IntMap.merge t1.children t2.children ~f:(fun ~key:_ v ->
                    match v with
                    | `Both _ ->
                        failwith
                          "Tree.merge cannot merge trees that share nodes"
                    | `Left v | `Right v -> Some v);
              parent =
                IntMap.merge t1.parent t2.parent ~f:(fun ~key:_ v ->
                    match v with
                    | `Both _ ->
                        failwith
                          "Tree.merge cannot merge trees that share nodes"
      ...
      in
      {
        ...
        children =
          IntMap.add_exn temp_tree.children ~key:parent
            ~data:(IntSet.of_list roots);
        parent =
          List.fold_left roots ~init:temp_tree.parent ~f:(fun t root ->
...
module Test = struct
  ...
  let%test "revert tree computation" =
    let entry : int = 0 in
    let tree : IntSet.t IntMap.t =
      IntMap.map ~f:IntSet.of_list
        (IntMap.of_alist_exn
           [
    ...
    let expected : int IntMap.t =
      IntMap.of_alist_exn [ (1, 0); (3, 1); (4, 1); (2, 0); (5, 2) ]
    in
  ...
  let%test "merging two different trees" =
    let t1 =
      of_children_map 0 [ (0, [ 1; 2 ]); (1, [ 3 ]); (2, []); (3, []) ]
    in
    let t2 =
      of_children_map 4 [ (4, [ 5 ]); (5, [ 6; 7 ]); (6, []); (7, []) ]
    in
