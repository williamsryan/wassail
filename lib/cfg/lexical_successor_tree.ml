...
type t = {
  ...
  block_entries : int Instr.Label.Map.t;
      (* Maps block labels to their instruction's index *)
  no_final_node : bool;
}
...
let make (instrs : unit Instr.t list) : t =
  ...
  let add_parent (roots : Tree.t list) (idx : int) : Tree.t =
    if List.is_empty roots then Tree.of_node idx
    else
      (* This node is a successor of all the roots that we have *)
      Tree.merge roots idx
  in
  let rec helper (instrs : unit Instr.t list) (roots : Tree.t list)
      (indices : unit Instr.t IntMap.t) (entries : int Instr.Label.Map.t) :
    ...
    | Control instr :: rest -> (
        ...
        | Merge ->
            failwith
              "lst: there should be no merge instructions before constructing \
               the CFG"
        | If (_, _, instrs1, instrs2) ->
            let roots1, indices'', entries' =
              helper instrs1 [] indices' entries
            in
            let roots2, indices''', entries'' =
              helper instrs2 [] indices'' entries'
            in
            let roots' = (add_parent roots idx :: roots1) @ roots2 in
            let entries''' =
              Instr.Label.Map.add_exn entries'' ~key:instr.label ~data:idx
            in
            helper rest roots' indices''' entries'''
        | Block (_, _, instrs) | Loop (_, _, instrs) ->
            let roots', indices'', entries' =
              helper instrs [] indices' entries
            in
  ...
  in
  let roots, indices, entries =
    helper instrs [] IntMap.empty Instr.Label.Map.empty
  in
  match roots with
  ...
  | _ ->
      let idx = next_idx () in
      {
        successors = add_parent roots idx;
        index_to_instr = indices;
        block_entries = entries;
        no_final_node = true;
      }
...
module Test = struct
  ...
  let%test "lexical successor tree of a sequence of instructions" =
    let t =
      make
        [
          data 1 (Instr.Const (Prim_value.of_int 0));
    ...
    in
    let expected =
      Tree.of_children_map 4 [ (4, [ 3 ]); (3, [ 2 ]); (2, [ 1 ]); (1, []) ]
    in
            ...
            (Instr.Block
               ( None,
                 ...
                 [
                   data 3 (Instr.Const (Prim_value.of_int 0));
                   control 4
                     (Instr.Block (None, (0, 0), [ control 5 (Instr.Br 0l) ]));
                 ] ));
          data 6 (Instr.Const (Prim_value.of_int 0));
        ]
    in
    (* 6 -> 2 -> 1
         -> 4 -> 3
          ...
          control 4
            (Instr.If
               ( None,
                 ...
                 [] ));
          data 7 (Instr.Const (Prim_value.of_int 0));
        ]
    in
    (* 7 -> 4 -> 3 -> 2 -> 1
         -> 6 -> 5 *)
