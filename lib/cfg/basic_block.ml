...
let to_string ?(annot_str : 'a -> string = fun _ -> "") (b : 'a t) : string =
  Printf.sprintf "block %d, %s" b.idx
    (match b.content with
    | Control instr ->
        Printf.sprintf "control block: %s"
          (Instr.control_to_string instr.instr ~annot_str)
    | Data instrs ->
        Printf.sprintf "data block: %s"
          (String.concat ~sep:"\\l"
...
let to_dot ?(annot_str : 'a -> string = fun _ -> "") (b : 'a t) : string =
  ...
  | Control instr ->
      Printf.sprintf
        "block%d [shape=Mrecord, label=\"{Control block \
         %d|%s<instr%s>%s:%s%s}\"];"
        b.idx b.idx
        (match annot_str instr.annotation_before with
        | "" -> ""
...
let all_instruction_labels (b : 'a t) : Instr.Label.Set.t =
  ...
  | Data d ->
      List.fold_left d ~init:Instr.Label.Set.empty ~f:(fun acc i ->
...
let all_annots (b : 'a t) : 'a list =
  ...
  | Data d ->
      List.fold_left d ~init:[] ~f:(fun acc i ->
          [ i.annotation_before; i.annotation_after ] @ acc)
...
let is_merge (b : 'a t) : bool =
  match b.content with Control { instr = Merge; _ } -> true | _ -> false
...
let is_data (b : 'a t) : bool =
  match b.content with Data _ -> true | _ -> false

(** Check if the block is a control block *)
let is_control (b : 'a t) : bool =
  match b.content with Control _ -> true | _ -> false

(** Check if the block is an empty block *)
let is_empty (b : 'a t) : bool =
  match b.content with Data [] -> true | _ -> false
