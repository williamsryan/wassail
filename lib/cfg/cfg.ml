...
module Edge = struct
  module T = struct
    type t = int * bool option [@@deriving sexp, compare, equal]
  end
...
module Edges = struct
  module T = struct
    type t = Edge.Set.t IntMap.t [@@deriving sexp, compare, equal]
  end
  ...
  let remove_from (edges : t) (from : int) : t = IntMap.remove edges from
end
...
let local_type (cfg : 'a t) (l : int32) =
  ...
  if Int32.(l < nargs) then List32.nth_exn cfg.arg_types l
  else List32.nth_exn cfg.local_types Int32.(l - nargs)

let to_string (cfg : 'a t) : string =
  Printf.sprintf "CFG of function %s" (Int32.to_string cfg.idx)

let to_dot ?(annot_str : 'a -> string = fun _ -> "") ?(extra_data : string = "")
    ?(include_edges : bool = true) (cfg : 'a t) : string =
  Printf.sprintf "digraph \"CFG of function %s\" {\n%s\n%s\n%s}\n"
    (Int32.to_string cfg.idx)
    (String.concat ~sep:"\n"
       (List.map (IntMap.to_alist cfg.basic_blocks) ~f:(fun (_, b) ->
            Basic_block.to_dot ~annot_str b)))
    (if include_edges then
       String.concat ~sep:"\n"
         (List.concat_map (IntMap.to_alist cfg.edges) ~f:(fun (src, dsts) ->
              List.map (Edge.Set.to_list dsts) ~f:(fun (dst, br) ->
                  Printf.sprintf "block%d -> block%d [label=\"%s\"];\n" src dst
                    (match br with
                    | Some true -> "t"
                    | Some false -> "f"
                    | None -> ""))))
...
let to_adjlist (cfg : 'a t) : string * string =
  let nodes =
    cfg.basic_blocks |> IntMap.to_alist
    |> List.map ~f:(fun (idx, block) ->
           let t, content =
             ...
             | Data instrs ->
                 ( "d",
                   List.map instrs ~f:(fun i -> Instr.data_to_string i.instr)
                   |> String.concat ~sep:":" )
           in
           Printf.sprintf "%d:%s:%s" idx t content)
    |> String.concat ~sep:"\n"
  in
  let adj =
    cfg.edges |> IntMap.to_alist
    |> List.map ~f:(fun (source, edges) ->
           edges |> Edge.Set.to_list
           |> List.map ~f:(fun (dest, branch) ->
                  Printf.sprintf "%d %d %s" source dest
                    (match branch with
                    | Some true -> "t"
                    | Some false -> "f"
                    | None -> "x"))
...
let find_block_exn (cfg : 'a t) (idx : int) : 'a Basic_block.t =
  ...
  | Some b -> b
  | None ->
      failwith (Printf.sprintf "Cfg.find_block_exn did not find block %d" idx)

let find_enclosing_block (cfg : 'a t) (label : Instr.Label.t) :
    Instr.Label.t option =
  Instr.Label.Map.find cfg.label_to_enclosing_block label

let find_enclosing_block_exn (cfg : 'a t) (label : Instr.Label.t) :
    'a Basic_block.t =
  (* TODO: return a label instead? *)
  match
    List.find (IntMap.to_alist cfg.basic_blocks) ~f:(fun (_, block) ->
        let labels = Basic_block.all_direct_instruction_labels block in
        Instr.Label.Set.mem labels label)
  with
  | Some (_, b) -> b
  | None -> failwith "find_enclosing_block did not find a block"

let rec find_nth_parent_block (cfg : 'a t) (instruction : Instr.Label.t)
    (n : int32) : Instr.Label.t option =
  match find_enclosing_block cfg instruction with
  | None ->
      if Int32.(n = 0l) then None (* break out of the function *)
      else failwith "Cfg.find_nth_parent_block_exn: break level is too high"
  | Some parent ->
      if Int32.(n = 0l) then Some parent
      else find_nth_parent_block cfg parent Int32.(n - 1l)
...
let rec non_empty_predecessors (cfg : 'a t) (idx : int) : int list =
  ...
  let non_empty =
    List.filter preds ~f:(fun pred ->
        let block = find_block_exn cfg pred in
        match block.content with Data [] -> false | _ -> true)
  in
...
let callees (cfg : 'a t) : Int32Set.t =
  ...
  IntMap.fold cfg.basic_blocks ~init:Int32Set.empty
    ~f:(fun ~key:_ ~data:block callees ->
      match block.content with
      | Control { instr = Call (_, _, n); _ } ->
          Int32Set.union (Int32Set.singleton n) callees
      | _ -> callees)

let callers (cfgs : 'a t Int32Map.t) (cfg : 'a t) : Int32Set.t =
  Int32Map.fold cfgs ~init:Int32Set.empty
    ~f:(fun ~key:caller ~data:cfg' callers ->
...
let all_instructions (cfg : 'a t) : 'a Instr.t Instr.Label.Map.t =
  IntMap.fold cfg.basic_blocks ~init:Instr.Label.Map.empty
    ~f:(fun ~key:_ ~data:block acc ->
      match block.content with
      | Control i ->
          Instr.Label.Map.add_exn acc ~key:i.label ~data:(Instr.Control i)
      | Data d ->
          List.fold_left d ~init:acc ~f:(fun acc i ->
...
let find_instr (instructions : 'a Instr.t Instr.Label.Map.t)
    (label : Instr.Label.t) : 'a Instr.t option =
  Instr.Label.Map.find instructions label

let find_instr_exn (instructions : 'a Instr.t Instr.Label.Map.t)
    (label : Instr.Label.t) : 'a Instr.t =
  match find_instr instructions label with
  | Some i -> i
  | None ->
      failwith
        (Printf.sprintf
           "Cfg.find_instr_exn did not find instruction with label %s"
...
let all_instruction_labels (cfg : 'a t) : Instr.Label.Set.t =
  IntMap.fold cfg.basic_blocks ~init:Instr.Label.Set.empty
    ~f:(fun ~key:_ ~data:block l ->
      Instr.Label.Set.union (Basic_block.all_instruction_labels block) l)

let all_annots (cfg : 'a t) : 'a list =
  IntMap.fold cfg.basic_blocks ~init:[] ~f:(fun ~key:_ ~data:block l ->
      Basic_block.all_annots block @ l)

let map_annotations (cfg : 'a t) ~(f : 'a Instr.t -> 'b * 'b) : 'b t =
  {
    cfg with
    basic_blocks =
      IntMap.map ~f:(fun b -> Basic_block.map_annotations b ~f) cfg.basic_blocks;
  }
...
let body (cfg : 'a t) : unit Instr.t list =
  List.map cfg.instructions ~f:(fun l ->
      Instr.Label.Map.find_exn cfg.label_to_instr l)

let rec state_before_block (cfg : 'a t) (block_idx : int) (entry_state : 'a) :
    'a =
  ...
  | Data [] -> (
      match non_empty_predecessors cfg block_idx with
      | [] -> (
          match predecessors cfg block_idx with
          | [] ->
          ...
          | _ ->
              failwith
                "state_before_block: multiple predecessors for an empty block")
      | pred :: [] ->
      ...
      | [] -> (
          ...
          | [] ->
              if block_idx <> cfg.entry_block then
                failwith
                  "state_after_block: state is empty and has no predecessor";
              entry_state
          | pred :: [] -> state_after_block cfg pred entry_state
          | _ ->
              failwith
                "state_after_block: multiple predecessors of an empty block")
      | pred :: [] ->
...
let replace_block (cfg : 'a t) (block : 'a Basic_block.t) : 'a t =
  {
    cfg with
    basic_blocks =
      IntMap.update cfg.basic_blocks block.idx ~f:(function
        | None ->
            failwith
              "Cfg.replace_block called with a block that did not exist in the \
               previous CFG"
        | Some _ -> block);
  }
...
let remove_block_rewrite_edges (cfg : 'a t) (block_idx : int) : 'a t =
  ...
  *)
  let without_edges =
    let edges' = Edges.remove_from cfg.edges block_idx in
    (* Remove all edges starting from the current block *)
    let srcs = Edges.from cfg.back_edges block_idx in
    (* Find all edges that go to this node *)
    List.fold_left srcs ~init:edges' ~f:(fun edges (src, _) ->
        (* and remove them *)
        Edges.remove edges src block_idx)
  in
  ...
  let new_edges =
    List.fold_left incoming_edges ~init:without_edges
      ~f:(fun edges (src, cond) ->
        List.fold_left outgoing_edges ~init:edges ~f:(fun edges (dst, _) ->
            if src <> dst then
              Edges.add edges src
                ( dst,
                  cond
                  (* Keep the first condition as the second block can't be executed anymore *)
                )
            else (* No self-cycle. *)
              edges))
  in
  ...
  let incoming_edges = Edges.from cfg.back_edges block_idx in
  let new_back_edges =
    List.fold_left incoming_edges ~init:without_back_edges
      ~f:(fun back_edges (src, cond) ->
        List.fold_left outgoing_edges ~init:back_edges
          ~f:(fun back_edges (dst, _) ->
            if src <> dst then Edges.add back_edges dst (src, cond)
            else back_edges))
  in
  {
    cfg with
...
let insert_block_between (cfg : 'a t) (src : int) (dst : int)
    (new_block : 'a Basic_block.t) : 'a t =
  let _, forward_annot = Edges.find_exn cfg.edges src dst in
  let edges =
    Edges.add
      (Edges.add
         (Edges.remove cfg.edges src dst)
         src
         (new_block.idx, forward_annot))
      new_block.idx (dst, None)
  in
  let _, backward_annot = Edges.find_exn cfg.back_edges dst src in
  let back_edges =
    Edges.add
      (Edges.add
         (Edges.remove cfg.back_edges dst src)
         dst (new_block.idx, None))
      new_block.idx (src, backward_annot)
  in
  let basic_blocks =
    match find_block cfg new_block.idx with
    | Some _ ->
        (* Block already present, do not add it *)
        cfg.basic_blocks
    | None -> IntMap.add_exn cfg.basic_blocks ~key:new_block.idx ~data:new_block
  in
...
let without_empty_nodes_with_no_predecessors (cfg : 'a t) : 'a t =
  let to_remove =
    IntMap.keys
      (IntMap.filter cfg.basic_blocks ~f:(fun block ->
           block.idx <> cfg.entry_block
           && List.is_empty (predecessors cfg block.idx)
           && Basic_block.is_empty block))
  in
  List.fold_left to_remove ~init:cfg ~f:(fun cfg idx ->
      remove_block_rewrite_edges cfg idx)

let all_predecessors (cfg : 'a t) (block : 'a Basic_block.t) :
    'a Basic_block.t list =
  let rec loop (worklist : IntSet.t) (visited : IntSet.t)
      (acc : 'a Basic_block.t list) : 'a Basic_block.t list =
    match IntSet.choose worklist with
    | None -> acc
    | Some block_idx when IntSet.mem visited block_idx ->
        loop (IntSet.remove worklist block_idx) visited acc
    | Some block_idx ->
        let preds = List.map ~f:fst (Edges.from cfg.back_edges block_idx) in
        let worklist' =
          IntSet.union (IntSet.of_list preds) (IntSet.remove worklist block_idx)
        in
        loop worklist'
          (IntSet.add visited block_idx)
          (find_block_exn cfg block_idx :: acc)
  in
...
let is_loop_exn (cfg : 'a t) (label : Instr.Label.t) : bool =
  ...
  | Some (Data _) ->
      failwith "Cfg.is_loop_exn: this is not a control instruction"
  | Some (Control i) -> (
      match i.instr with
      | Loop _ -> true
      | Block _ | If _ -> false
...
let is_block_exn (cfg : 'a t) (label : Instr.Label.t) : bool =
  ...
  | Some (Data _) ->
      failwith "Cfg.is_block_exn: this is not a control instruction"
  | Some (Control i) -> (
      match i.instr with
      | Block _ -> true
      | Loop _ | If _ -> false
