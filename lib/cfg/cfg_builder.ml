...
let compute_block_arities (instrs : unit Instr.t list) :
    (int * int) Instr.Label.Map.t =
  let rec loop (instrs : unit Instr.t list)
      (acc : (int * int) Instr.Label.Map.t) =
    match instrs with
    | [] -> acc
    ...
    | Control i :: rest ->
        loop rest
          ...
          | If (_, arity, then_, else_) ->
              loop else_
                (loop then_ (Instr.Label.Map.set acc ~key:i.label ~data:arity))
          | _ -> acc)
  in
  loop instrs Instr.Label.Map.empty

let compute_enclosing_blocks (instrs : unit Instr.t list) :
    Instr.Label.t Instr.Label.Map.t =
  let add (instr : Instr.Label.t) (current_block : Instr.Label.t option)
      (mapping : Instr.Label.t Instr.Label.Map.t) =
    match current_block with
    | Some l -> Instr.Label.Map.set mapping ~key:instr ~data:l
    | None -> mapping
  in
  let rec loop (instrs : unit Instr.t list)
      (current_block : Instr.Label.t option)
      (acc : Instr.Label.t Instr.Label.Map.t) =
    match instrs with
    | [] -> acc
    | Data i :: rest -> loop rest current_block (add i.label current_block acc)
    | Control i :: rest ->
        let acc = add i.label current_block acc in
        loop rest current_block
          (match i.instr with
          | Block (_, _, body) -> loop body (Some i.label) acc
          | Loop (_, _, body) -> loop body (Some i.label) acc
          | If (_, _, then_, else_) ->
...
let build (module_ : Wasm_module.t) (fid : Int32.t) : unit Cfg.t =
  ...
  let rec check_no_rest (rest : 'a Instr.t list) : unit =
    match rest with
    | [] -> ()
    | Control { instr = Unreachable; _ } :: rest -> check_no_rest rest
    | _ ->
        Log.info
          (Printf.sprintf "Ignoring unreachable instructions after jump: %s"
             (Instr.list_to_string rest (fun _ -> "")))
  in
  ...
  let new_idx () : int =
    ...
    v
  in
  let mk_data_block (reverse_instrs : (Instr.data, unit) Instr.labelled list) :
      unit Basic_block.t =
    let instrs = List.rev reverse_instrs in
    Basic_block.{ idx = new_idx (); content = Data instrs }
  in
  let mk_control_block (instr : (unit Instr.control, unit) Instr.labelled) :
      unit Basic_block.t =
    Basic_block.{ idx = new_idx (); content = Control instr }
  in
  let mk_merge_block () =
    let idx = new_idx () in
    Basic_block.
  ...
  let mk_empty_block () : unit Basic_block.t =
    Basic_block.{ idx = new_idx (); content = Data [] }
  in
  let loop_heads = ref IntSet.empty in
  let entry_exit = ref [] in
  (* TODO: can be removed *)
  let rec helper (instrs : (Instr.data, unit) Instr.labelled list)
    ...
    | Control instr :: rest -> (
        ...
        | Merge ->
            failwith
              "cfg_builder: There should be no merge instructions before \
        ...
        | BrIf level ->
            ...
            let blocks, edges, breaks, returns, entry', exit' =
              helper [] rest
            in
            ( (* add the new blocks *)
              block :: brif_block :: blocks,
              (* add an edge between this block, brif (false branch), and finally the rest *)
              (block.idx, brif_block.idx, None)
              :: (brif_block.idx, entry', Some false)
              :: edges,
        ...
        | If (_bt, _arity, instrs1, instrs2) ->
            ...
            let blocks, edges, breaks, returns, then_entry, then_exit =
              helper [] instrs1
            in
            let blocks', edges', breaks', returns', else_entry, else_exit =
              helper [] instrs2
            in
            ...
            let new_breaks =
              List.map
                (List.filter all_breaks ~f:(fun (_, level, _) ->
                     Int32.(level > 0l)))
                ~f:(fun (idx, level, branch) ->
                  (idx, Int32.(level - 1l), branch))
            in
            let break_edges =
              List.map
                (List.filter all_breaks ~f:(fun (_, level, _) ->
                     Int32.(level = 0l)))
                ~f:(fun (idx, _, branch) -> (idx, merge_block.idx, branch))
            in
            ( block :: if_block :: merge_block :: (blocks @ blocks' @ blocks''),
              (* Edges *)
              (block.idx, if_block.idx, None)
              :: (if_block.idx, then_entry, Some true)
              :: (if_block.idx, else_entry, Some false)
              :: (then_exit, merge_block.idx, None)
              :: (else_exit, merge_block.idx, None)
              :: (merge_block.idx, entry, None)
              :: (break_edges @ edges @ edges' @ edges''),
        ...
        | Br level ->
            ...
            let blocks, edges, breaks, returns, _entry', exit' =
              helper [] rest
            in
            ( block :: br_block :: blocks (* add the current block *),
              (block.idx, br_block.idx, None)
              :: edges (* only sequential edge *),
        ...
        | BrTable (table, level) ->
            ...
            ( block :: br_block :: blocks (* add the current block *),
              (block.idx, br_block.idx, None)
              :: edges (* only sequential edge *),
              (br_block.idx, level, None)
              :: List.map table ~f:(fun lvl -> (br_block.idx, lvl, None))
              @ breaks (* add all the breaks *),
              returns,
              block.idx,
              exit' )
        | Call _ | CallIndirect _ ->
            ...
            ( (* add the current block and the function block *)
              ...
              (block.idx, call_block.idx, None)
              :: (call_block.idx, entry', None)
              :: edges,
        ...
        | (Loop (_bt, _arity, instrs') as b) ->
            ...
            let is_loop = match b with Loop _ -> true | _ -> false in
            (* The block entry *)
            let block_entry =
              if is_loop then mk_merge_block () else mk_empty_block ()
            in
            if is_loop then loop_heads := IntSet.add !loop_heads block_entry.idx;
            ...
            let block_exit =
              if is_loop then mk_empty_block () else mk_merge_block ()
            in
            ...
            let new_breaks =
              ...
              @ (List.map
                   (List.filter all_breaks ~f:(fun (_, level, _) ->
                        Int32.(level > 0l)))) ~f:(fun (idx, level, branch) ->
                    (idx, Int32.(level - 1l), branch))
            in
            let break_edges =
              List.map
                ...
                ~f:(fun (idx, _, branch) ->
                  ( idx,
                    (if is_loop then block_entry.idx else block_exit.idx),
                    branch ))
            in
            ...
            let new_edges =
              if is_loop then
                [
                  (block.idx, block_entry.idx, None);
                  (block_entry.idx, entry', None);
                  (exit', block_exit.idx, None);
                  (block_exit.idx, entry'', None);
                ]
              else
                [
                  (block.idx, block_entry.idx, None);
            ...
            in
            ( block :: block_entry :: block_exit :: (blocks @ blocks')
              (* add all blocks *),
        ...
        | Unreachable ->
            ...
            ( block :: unreachable_block :: blocks,
              (block.idx, unreachable_block.idx, None) :: edges
              (* The unreachable block is not connected to anything except to the "return" block of the CFG *),
              breaks,
              unreachable_block.idx :: returns,
              block.idx,
              exit' ))
  in
  let blocks, edges, breaks, returns, _entry_idx, exit_idx =
    helper [] funcinst.code.body
  in
  ...
  let breaks_to_exit, remaining_breaks =
    List.partition_tf breaks ~f:(fun (_, lvl, _) -> Int32.(lvl = 0l))
  in
  if not (List.is_empty remaining_breaks) then
    (* there shouldn't be any breaks outside the function *)
    Log.warn
      (Printf.sprintf "There are %d breaks outside of function %ld"
         (List.length breaks) fid);
  ...
  let edges' =
    ...
    :: List.map breaks_to_exit ~f:(fun (block_idx, _, data) ->
           (block_idx, return_block.idx, data))
    @ List.map returns ~f:(fun from -> (from, return_block.idx, None))
    @ List.filter edges ~f:(fun (src, _, _) ->
          not (List.mem returns src ~equal:Stdlib.( = )))
  in
  (* We now filter empty normal blocks *)
  let actual_blocks, filtered_blocks =
    List.partition_tf blocks' ~f:(fun block ->
        match block.content with
        | Data [] ->
            (* only filter if we need to simplify, and it is an empty data block which is not a block/loop entry or exit *)
            if simplify then false else true
        | _ -> true)
  in
  ...
  let actual_edges =
    List.fold_left filtered_blocks_idx ~init:edges' ~f:(fun edges idx ->
        (* idx is removed, so we find all edges pointing to idx (and we keep track of the other ones, as only these should be kept) *)
        let pointing_to, edges' =
          List.partition_tf edges ~f:(fun (_, dst, _) -> dst = idx)
        in
        (* and we find all edges pointing from idx (again, keeping track of the other ones) *)
        let pointing_from, edges'' =
          List.partition_tf edges' ~f:(fun (src, _, _) -> src = idx)
        in
        (* now we connect everything from both sets *)
        List.concat
          (List.map pointing_to ~f:(fun (src, _, data) ->
               List.map pointing_from ~f:(fun (_, dst, data') ->
                   ...
                   | Some _, Some _ ->
                       failwith
                         "trying to merge two conditional edges, should not \
                          happen")))
        @ edges'')
  in
  ...
  let first_block =
    Option.value_exn
      (List.min_elt
         (List.map actual_blocks ~f:(fun b -> b.idx))
  ...
  let entry_block, actual_blocks', actual_edges' =
    ...
    | Some _ ->
        ...
        ( block.idx,
          block :: actual_blocks,
          (block.idx, first_block, None) :: actual_edges )
  in

  let edges =
    IntMap.map
      (IntMap.of_alist_multi
         (List.map actual_edges' ~f:(fun (src, dst, data) -> (src, (dst, data)))))
      ~f:(fun es -> Cfg.Edge.Set.of_list es)
  in
  let back_edges =
    IntMap.map
      (IntMap.of_alist_multi
         (List.map actual_edges' ~f:(fun (left, right, data) ->
              (right, (left, data)))))
      ~f:(fun es -> Cfg.Edge.Set.of_list es)
  in
  let basic_blocks =
    IntMap.of_alist_exn (List.map actual_blocks' ~f:(fun b -> (b.idx, b)))
  in

  let label_to_instr =
    let rec loop (instrs : unit Instr.t list)
        (acc : unit Instr.t Instr.Label.Map.t) : unit Instr.t Instr.Label.Map.t
        =
      match instrs with
      | [] -> acc
      ...
      | (Control { instr = If (_, _, instrs1, instrs2); label; _ } as i) :: rest
        ->
          loop
            (rest @ instrs1 @ instrs2)
            (Instr.Label.Map.add_exn acc ~key:label ~data:i)
      | i :: rest ->
          loop rest (Instr.Label.Map.add_exn acc ~key:(Instr.label i) ~data:i)
    in
    loop funcinst.code.body Instr.Label.Map.empty
  in
  Cfg.
    {
...
module Test = struct
  ...
  let test_cfgs file =
    ...
    List.iteri wasm_mod.funcs ~f:(fun i _ ->
        let faddr = Int32.(wasm_mod.nfuncimports + Int32.of_int_exn i) in
        let _ : unit Cfg.t = build wasm_mod faddr in
        ())

  let%test_unit "CFG for simple.wat can be built" =
    test_cfgs "../../../test/simple.wat"

  let%test_unit "CFG for if-loop.wat can be built" =
    test_cfgs "../../../test/if-loop.wat"

  let%test_unit "CFG for rel.wat can be built" =
    test_cfgs "../../../test/rel.wat"

  let%test_unit "CFG for memcpy.wat can be built" =
    test_cfgs "../../../test/memcpy.wat"

  let%test_unit "CFG for loop.wat can be built" =
    test_cfgs "../../../test/loop.wat"

  let%test_unit "CFG for loop-brif.wat can be built" =
    test_cfgs "../../../test/loop-brif.wat"
end
