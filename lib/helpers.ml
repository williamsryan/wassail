...
module IntSet = struct
  ...
  let to_string (s : t) : string =
    String.concat ~sep:"," (List.map (Set.to_list s) ~f:string_of_int)
end
...
module Int32Set = struct
  ...
  let to_string (s : t) : string =
    String.concat ~sep:"," (List.map (Set.to_list s) ~f:Int32.to_string)
end
...
module IntPairSet = struct
  ...
  let to_string (s : t) : string =
    String.concat ~sep:","
      (List.map (Set.to_list s) ~f:(fun (x, y) -> Printf.sprintf "(%d,%d)" x y))
end
...
module IntMap = struct
  ...
  let to_string (m : 'a t) (f : 'a -> string) : string =
    String.concat ~sep:", "
      (List.map (Map.to_alist m) ~f:(fun (k, v) ->
           Printf.sprintf "%d → %s" k (f v)))
end
...
module Int32Map = struct
  ...
  let to_string (m : 'a t) (f : 'a -> string) : string =
    String.concat ~sep:", "
      (List.map (Map.to_alist m) ~f:(fun (k, v) ->
           Printf.sprintf "%s → %s" (Int32.to_string k) (f v)))
end
...
let error at category msg =
  failwith
    (Printf.sprintf "Error: %s"
       (Wasm.Source.string_of_region at ^ ": " ^ category ^ ": " ^ msg))
...
let apply_to_string str run =
  parse_from_lexbuf_textual "no-file" (Lexing.from_string str) run
...
let apply_to_script_file (filename : string) (cmd : Wasm.Script.command' -> 'a)
    : 'a list =
  In_channel.with_file filename ~f:(fun ic ->
      let lexbuf = Lexing.from_channel ic in
...
let apply_to_file (filename : string) (f : Wasm.Ast.module_ -> 'a) : 'a =
  ...
  | ".wast" ->
      List.hd_exn
        (List.filter_map
           (apply_to_script_file filename (function
             | Wasm.Script.Module (_, { it = Wasm.Script.Textual m; _ }) ->
                 Some (f m)
             | _ -> None))
           ~f:(fun x -> x))
  | ext ->
      Printf.printf
        "Invalid extension for WebAssembly module: %s. Assuming .wat extension\n"
        ext;
...
module Product_domain = struct
  module Make (Domain1 : Abstract_domain) (Domain2 : Abstract_domain) = struct
    type t = Domain1.t * Domain2.t [@@deriving sexp, compare, equal]
    ...
    let join (t1 : t) (t2 : t) : t =
      (Domain1.join (fst t1) (fst t2), Domain2.join (snd t1) (snd t2))
  end
end
