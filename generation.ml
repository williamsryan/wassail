...
let generate =
  Command.basic ~summary:"Generate a WebAssembly module from a single function"
    Command.Let_syntax.(
      ...
      fun () ->
        ...
        let module_ =
          List.fold_left module_.funcs ~init:module_ ~f:(fun m f ->
              ...
              else Wasm_module.remove_func m f.idx)
        in
