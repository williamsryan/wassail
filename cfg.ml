...
let cfg =
  Command.basic
    ~summary:
      "Generate a DOT file representing the CFG of function [fid] from the wat \
       file [in], in file [out]"
    Command.Let_syntax.(
...
let cfg_adjlist =
  Command.basic
    ~summary:
      "Generate the CFG of function [fid] from the wat file [in], in two text \
       files: [out].adjlist and [out].nodes"
    Command.Let_syntax.(
      let%map_open file_in = anon ("in" %: string)
      and fid = anon ("fid" %: int32)
      and file_out = anon ("out" %: string) in
...
let cfgs =
  Command.basic
    ~summary:
      "Generate DOT files representing the CFG of each function defined in the \
       wat file [in], and outputs them in the directory [out_dir]"
    Command.Let_syntax.(
      let%map_open file_in = anon ("in" %: string)
      ...
      fun () ->
        ...
        List.iteri wasm_mod.funcs ~f:(fun i _ ->
            ...
            Out_channel.with_file (Printf.sprintf "%s/%ld.dot" out_dir faddr)
              ~f:(fun ch -> Out_channel.output_string ch (Cfg.to_dot cfg))))
